...
```text
1. 项目目标
创建《TITANIC — THE LAST LIGHT》：一段时长 264 秒的互动电影式航行，从船只最后的日落开始，经过碰撞、撤离和沉没，直至黎明纪念。访客可以观看导演式影片、探索持续运动的 3D 世界、跳转章节、保存静帧或下载完整影片。将其呈现为艺术化诠释，不宣称具备法证准确性，也不暗示与官方存在关联。

2. 视觉方向
采用克制的电影感调色：深邃的大西洋蓝映衬温暖的奶油色与琥珀色船灯，随后转入星光点点的暗夜和清冷的黎明。使用透视场景，采用 2.39:1 电影构图、柔和泛光、细微颗粒和暗角。为人物肖像和黎明场景使用景深，同时保持遇险信号火箭的粒子清晰。海洋主体使用深蓝色，加入世界空间波浪、较小的 mipmap 纹理涟漪法线和菲涅耳反射；温暖的日落色彩主要应来自反射光。使用附着于船体、跟随相同海洋位移的尾流带，边缘柔和并带有断续泡沫。水平平铺全景纹理时不要出现分形断裂，避免天空出现垂直接缝和反射条纹。避免橙色浅水着色、细小且均匀的涟漪，以及发光的圆形泡沫贴花。移除隐藏的重叠屋顶顶面，防止深度冲突；根据镜头距离设置合适的相机近裁剪面。沉没过程中，让窗光单调变暗，不要加入高频闪烁。使用安静的衬线标题、双语中英文控件，以及沿底部排列的窄时间线。

3. 世界、地理与镜头剪辑
使用统一且连续的坐标系：船长 269 米，船首朝向 +X，冰山固定在 (275, 0, 57)。船只向前航行，在 96.727 秒时接触冰山，滑行至停止，随后船首和船尾分段下沉。冰山要一直保留到结尾，并出现在黎明构图中。保留六个章节，起始时间分别为 0、63、110、163、211 和 241 秒。

制作 23 个经过刻意设计的镜头。船首相拥段落持续 29–61 秒：包括建立镜头式接近、双人面部特写、从身后望向海面的视角，以及斜侧人物肖像。将 Rose 放在前方、Jack 放在她身后，二人位于船首最前端，均面向船首外侧的海面。整个段落保持日落光照。从 127–158 秒，使用绑定到救生艇实际世界变换的三个镜头：从登艇甲板出发、近距离拍摄乘客和悬挂绳索，以及接近水面。随后安排宽幅撤离、船体倾斜、断裂和沉没镜头。黎明时展示幸存的救生艇和远处的冰山，然后以克制的纪念标题收束。

4. 资产清单
- titanic-vessel：在 Blender 中制作 269 米长的主体结构，包括连续的井甲板、封闭式艏楼、分层的 promenade 甲板、四个中空且后掠的浅黄色烟囱、黑色上部船体和红色下部船体。为 864 个舷窗使用真实圆形边框，为 360 个窗户制作窗框。仅允许实际玻璃材质使用自发光。批处理材质，并在 x=-32 处分割，以便实现沉没。添加桅杆、索具、吊艇架、吊索、青铜螺旋桨和舵。制作带柚木门和黄铜细节的 P2 楼梯通道，将其规格归一化，并在已制作的甲板上复用两次。组装 GLB 时保留节点变换。
- atlantic-iceberg：一座不规则、经过侵蚀的蓝白色冰山，具有分层霜面、粗糙度变化、克制的法线贴图和可信的水线。它是固定的地理对象。
- lifeboat：一艘 White Star 划艇，配有白色木质船壳、深色舷缘、长椅和船桨；将其实例化为 16 艘可独立移动的船。
- bow-embrace：一个独立的双人物资产，人物服装和姿势参考所要求的 1997 年电影：Rose 为栗色头发，穿海军蓝/象牙色服装并披着印花披肩，双臂伸展；Jack 紧贴其后，穿深色外套和象牙色衬衫。转换为 3D 前，先生成干净的参考图。根据干净的头部—颈部—肩部参考图，为每位主角将近景头部、完整颈部和上胸制作成一块连续的 H v3.1 表面。保留 P2 相拥姿势、手部和服装。使用四边面重拓扑、Blender 局部表面清理，以及 4K 颜色/法线烘焙。将接口贴合在衣物开口下方，不要直接在下颌下方连接。保留生成的连续颈部。根据测量出的躯干轮廓，将下胸部贴合到原始服装下方；恢复连续的 P2 上背部，并移除过时的头发残留。避免漂浮的衣领面片和拉伸的 UV 颈部挤出。检查两个低侧视图和后视图，确认没有漏光间隙或暴露的切边。从已制作的相机视角检查眼睑、虹膜、嘴唇、手部和纹理接缝。将布料掠射角高光与皮肤着色分开降低。使用自然的基础表情，并保持身体和布料运动克制；除非实际实现了面部动画骨骼，否则不要暗示存在面部动画绑定。
- seated-woman 和 seated-man：分开的成年乘客模型，穿 1912 年服装和浅色软木救生衣，屈膝坐姿，双手放在腿上。在各艘船之间共享几何体和材质；略微变化摆放位置与朝向。使用可逆的每船实例数量控制登船人数。

主船使用 Blender，甲板楼梯通道、冰山、船只、服装和乘客使用 Tripo P2.0，两个连续的主角肖像使用 H v3.1。海洋、天空混合、星星、灯光、烟雾、遇险火箭、泡沫、水花和碎片均作为场景特效保留。提供带压缩纹理的轻量 Web 模型变体，同时保留完整分辨率资产，用于离线影片导出。

5. 播放与反馈
为必要的船只、救生艇、天空和海面法线资产显示真实加载进度。第一场景准备就绪后启用开始按钮；其他模型和音乐延后加载。如果必需的人物或冰山模型延迟，就停在其场景边界，准备好后再继续，不要静默跳过对应镜头。音频在用户交互后开始。

时间线必须支持前后拖动定位和快速拖拽，且不能重置为零。定位时保留播放/暂停和静音状态；不要让旧音频时钟覆盖请求的位置。为 MP3 和 MP4 提供字节范围响应。章节导航包括 29 秒的船首相拥和 127 秒的救生艇下放的直接入口；这些入口会恢复导演相机，同时保留播放状态。

探索模式允许旋转、拖动和缩放，同时世界、船只和配乐继续运行。跟随船只平移，但不要让观察方向突然跳变。暂停状态保持独立；返回影片时保留当前时间。空格键播放/暂停，方向键跳转十秒，M 切换声音，E 切换探索模式，F 打开全屏。支持触控旋转/双指缩放和点击时间线。

救生艇初始为空。112 秒后，乘客分批登船，并在每艘船下沉前完成登船。绳索连接移动的吊艇架与船只实际连接点，释放后消失。向后定位时恢复较早的乘客占用和绳索状态。碰撞必须协调船体/相机震动、冰屑、刮擦水花，以及钢铁与冰接触时的瞬态音效。遇险火箭使用白色燃烧星体、短而独立的尾迹、重力、阻力和逐渐淡出的烟雾。沉没扰动应是不规则、跟随波浪的斑块，并逐渐衰减；水花沿船尾真实水线分布，绝不能从远处的点状喷泉发射。

6. 技术实现与交付物
使用 Vite、JavaScript 模块和 Three.js，实现基于确定性时间的动画。将相机/时间线、船只资产、人物、环境、特效和场景就绪状态分开管理。Web 播放、定位和离线采集使用相同的时间模型。将 Web 渲染控制在明确的像素、反射和阴影预算内；更高成本的环境光遮蔽延后到离线配置。编译并解码资产时，不要使用漫长的阻塞式启动预热。脚本、模型、图片、字体和音频托管在同一源下，并确保静态构建中不包含机密信息。

使用原创配乐和付费 ElevenLabs Foley：将完整、自然的口哨烟花录音按实际空爆点拆分为上升飞行段和带噼啪余响的清脆爆响，并制作钢铁/冰接触与刮擦、救生艇绳索与水面接触、船体受力/断裂，以及船尾排水声。导出源 WAV 文件，保留提示词/历史记录 ID，并剪辑成带时间点的提示音效。在 119、151 和 183 秒对齐发射时间，空爆分别延后 3.15 秒发生。保留原始爆响的起始冲击，并让管弦乐配乐短暂降低音量。混音中保持安静的海面、风声和引擎环境音。未经授权，不得在公开网站和可下载影片中包含参考电影的原声带。记录实际的资产和声音来源，不要将备用方案描述为服务生成的资产。

提供确定性导出：共 6,336 帧，分辨率 3840×2160，帧率 24 fps，使用三个时间采样，叠加英文标题，并输出 2.39:1 信箱式画面。编码一份 4K H.264/AAC 母版和一份小于 100 MiB 的 1080p Web 版本；两者时长均为 264 秒，音频均为 48 kHz 立体声。保留可选的中英文字幕、音频母版和可编辑源文件。通过现有 CMS Web Pages 托管发布静态构建，不要添加平台应用或按页面配置 Worker。

7. 验收
检查开场、两个人物肖像、碰撞、遇险火箭、全部三个下放镜头、断裂、船尾消失和黎明场景。确认冰山在 244 秒时不会消失，海洋呈现深蓝色，沉没过程中没有规则的白色圆环，人物在其制作的镜头中始终存在，船只下放全过程中绳索和乘客始终对齐。测试模型延迟加载、前后定位、快速拖动、暂停/静音、动态探索、直接进入特写，以及触控模拟。核验每一帧导出结果，完整解码两部影片，将实际浏览器下载内容与交付文件进行比对，并验证公开构建和 CMS 关联。区分浏览器移动端模拟与实体手机测试。
