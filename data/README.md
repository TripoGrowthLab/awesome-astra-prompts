...
`media.image` is a repository-relative WebP path into `assets/previews/`. When the source has a video, its full-length CMS-hosted URL is retained separately as `media.video`; the image is its source thumbnail or an extracted frame. Optional `media.previewVideo` provides a shorter CMS-hosted preview for lightweight playback and never replaces the full video or the README's Watch video link. `assets/manifest.json` records public image origins and checksums for attribution and integrity. A media reference is not a redistribution license.
