...
<a href="https://www.tripo3d.ai/ja/3d-prompts/bubble-bay"><img src="../assets/previews/3f9d3b8fcab4a54ff4bf19c6f25eb3f3ce6ce697cb929ab0abf86e89d6b8f708.jpg" width="840" loading="lazy" alt="バブルベイ：3D水風船バトル"></a>

<a href="https://www.tripo3d.ai/ja/3d-prompts/bubble-bay"><img src="../assets/previews/a0927800e707bb993467cd9c9d32d7955cafcad0fe1ab3986c33d6bec6ce85f8.jpg" width="840" loading="lazy" alt="バブルベイ：3D水風船バトル"></a>

<a href="https://www.tripo3d.ai/ja/3d-prompts/bubble-bay"><img src="../assets/previews/44f0d05c544d89c5044aece0e9da8cc3730bc51d81f395ff0e554bbceb4f2ab2.jpg" width="840" loading="lazy" alt="バブルベイ：3D水風船バトル"></a>

Pirate14とVillage10を舞台に、3体のアニメーション付きP2キャラクターで遊ぶ、クラシックな3D水風船バトルを構築します。迫力のある近距離視点、虹色に輝くトラップバブル、設定可能な捕獲目標、AIライバル、カスタムキャラクターのインポート、オリジナルのElevenLabs音楽に対応します。捕獲数の初期値は6回とし、読み込み完了後に比較タブを表示します。

**プロンプト**

```text
Bubble Bay――クラシックなBnBを思わせる、プレイ可能なThree.js製の水風船アリーナを構築します。初期状態では詳細なTripoモデルを使用し、対戦内容を維持したまま切り替えられる、分かりやすいThree.jsジオメトリ比較スイッチを用意します。Boz/Bazzi、Ike/Uni、Plunk/Keffyの提供済みオリジナルキャラクター資料を使用し、顔の特徴、帽子などの装具、色、衣装、体型の比率を維持してください。雰囲気だけを参考にしたテキスト生成デザインへ置き換えないでください。

キャラクターとアイテムのジオメトリは、明示的にtripo-p2を指定してTripo CLIで生成します。正面を向いたニュートラルな基準ポーズを用意してモデルを生成し、実際の二足歩行スケルトンをバインドして正しいスキンウェイトを設定し、GLBで書き出します。タスクID、クレジット、元のスキン付きファイル、最終ハッシュを保持します。待機・走行・ジャンプは、実際のジョイントが動くアニメーションにしてください。頭、耳、手袋が大きくずれるスキニングを修正し、複数の視点と動作を確認します。ローカルで作成したクリップは、Tripoのプリセットとしてではなく、正確に説明してください。オリジナルキャラクターの一般利用権については別途確認が必要です。AI生成は利用許諾を意味しません。

ローカライズ名：宝宝 / 배찌 / Boz (Bazzi)、小乖 / 우니 / Ike (Uni)、胖墩 / 케피 / Plunk (Keffy)。開始時の容量／射程／速度レベル：1/1/5、1/2/5、1/2/4。上限：6/7/9、6/7/8、9/8/8。速度は0.25 + レベル*0.8ワールド単位／秒に変換し、タイルは2単位とします。キャラクターを選ぶと、残り2体が異なるライバルとして割り当てられ、対応するプロフィールが適用されます。

デフォルトで15x13のPirate/Patrit14とVillage10を用意します。見分けやすい金色のデッキ、黄色い貨物、木箱4個、4門の大砲、中央のマストを維持してください。村には、色分けされた4つの住宅地区、中央の道路、生け垣、積み木を配置します。パブリッシャーのマップを参照しつつ、実行時のアートワークは自分で構築し、3Dでの継続的な移動とAIの逃走に必要な小さな通路の開口部を記録します。明るいマテリアル、影、海の風景、分かりやすいカメラ追従、全体を見渡せるビューを提供します。

1人のプレイヤーが、協力する2体のAIライバルと対戦します。WASD／矢印キーで移動、Fで2.5秒間持続するバブルを設置、Spaceで実際の足場にジャンプ、Shiftでダッシュ、Q/Eで旋回、Vで視点変更、Escapeで一時停止します。十字型に広がる水は障害物に沿って進み、最初の壊せるブロックを破壊してバブルを連鎖させます。捕獲、脱出、敵の捕獲、リスポーン、捕獲目標3・6・9・12回（初期値は6回）、または180秒間のスコア制、結果表示、リトライを実装します。タッチジョイスティックとアクションボタンは同時に操作できるようにします。

生成したTripoアイテムを6種類使用します：風船、射程ポーション、ローラースケート、投げ手袋、キックブーツ、救助ニードル。木箱からは85%の確率でアイテムが出現します。条件付きアイテム出現率は30/30/30/2.5/3.5/4%です。手袋で投げられる回数を3回増やし、上限は6回とします。Gで近くのバブルを遮蔽物越しに最大4タイル先まで投げられるようにします。バブルの所有者と元の導火時間を維持し、着地点を予約して軌道を表示します。飛行中に導火時間が切れたバブルは着地して爆発します。Kでバブルを押し、障害物に当たるまで滑らせます。導火時間はリセットしません。Xで救助ニードルを使用します。開始数は1本、上限は3本です。インベントリと使用可能な操作を明確に表示します。

UIは中国語、英語、韓国語に対応します：中国本土のIANAタイムゾーンでは中国語、韓国／北朝鮮では韓国語、それ以外（香港・マカオ・台湾を含む）では英語を選択します。手動選択は常に優先されます。埋め込みテクスチャを使用し、標準の非圧縮形式で、実際にスキニングされたGLBのみをアップロード可能にします。上限は40 MB、150kトライアングルです。ボーン、ジョイント、ウェイトを検証し、静的モデルは拒否します。アニメーションが埋め込まれている場合はそれを使用し、クリップがない認識済みヒューマノイドには基本的なジョイント動作を提供します。認識できないスケルトンにはアニメーションが必要です。リギングとアニメーションの違いを平易に説明し、90度の向き調整を用意します。処理はブラウザ内でローカルに行い、ファイルハッシュに基づいて安定したバランスの良いステータスを割り当てます。

作成CTAには必ずhttps://studio.tripo3d.ai/?utm_source=satellite_invite&utm_medium=bubble-bay&utm_campaign=create-characterを使用します。jaredのクレジットはhttps://x.com/jaredliu_bravoにリンクします。

ElevenLabsの効果音を13種類統合し、ノーマライズ、短い減衰、距離／パン、同時発音数の制限、音量調整、ミュートに対応します。効果音は、設置、破裂、木箱、アイテム取得、レアアイテム、投げ、着地、トラップ、救助、ジャンプ、勝利、敗北、キックを用意します。ElevenLabs music_v2_5でオリジナルのインストゥルメンタルトラックを2曲作成します。各90秒で、航海をテーマにした海賊マップと、明るい住宅街に合う内容にしてください。控えめな-20 LUFSを目標にノーマライズし、ループ境界をクロスフェードします。音楽専用の音量調整を用意し、マップごとに曲を切り替え、ポーズ時にフェードし、マスターのミュート設定に従います。元ゲームのサウンドトラックは再現しないでください。

独立したソース、依存関係、テスト、アセットの出所情報はCMSプラットフォームのアプリ外で管理します。同一オリジンの静的リソースとMITライセンス表記を同梱します。Ego Liteで、デスクトップおよびタッチ操作の実際のゲームプレイ、アップロード形式、言語、表示切り替えを検証します。既存のCMS Web Page12に紐付けた変更不能なレビュー版を作成し、レビュー中は既存の公開版を維持します。正確なソース情報とアセット権利の確認が済んだ場合のみ公開し、その後、公開URL、モデルハッシュ、動作を検証します。履歴を保持し、保存済みCMSレコード、プレビュー、公開版を区別します。

モデルアセットの読み込み状況を表示し、同時ダウンロード数は最大3、アイドルタイムアウトは30秒、試行回数は2回とします。再試行時も、正常にダウンロード済みのファイルは保持します。すべてのモデルの準備が完了するまでTripo / Three.jsタブ切り替え全体を非表示にし、完了後に表示します。手続き的に読み込んだプレースホルダーを、読み込み済みのTripoモデルとして表示しないでください。


アリーナの中央を向き、スポーン地点の外側から開始する、近距離の追従カメラを使用します。Q/Eの旋回、マウスドラッグによるピッチ操作、Vの全体表示を維持します。上部にはコンパクトなスコアボード、左上にはモード切り替え、下部付近には短い通知を配置します。マウス操作のデスクトップではジョイスティックと大きなタッチ操作ボタンを隠し、使用頻度の低いアイテム用の小さな操作は残します。狭い画面では、コンパクトなタッチ操作を同時に実行できるようにします。深度がティール色に変化するアニメーション付きの波シェーダー、有機的な砂地の島、曲がったヤシの木と岩、控えめな木目と草のテクスチャを追加します。捕らえられた各キャラクターを、サイズに合わせた半透明のバブルで包み、虹色に輝くフレネルリム、穏やかな浮遊、小さな泡、地面の波紋を表示します。ポーズ中のスキン頂点が適切に収まっていることを確認し、救助時に浮き上がりをリセットします。切り替えを繰り返した後のリソース破棄も確認します。水の膜に対して高コストなシーン全体の透過パスは避け、狭い画面ではピクセル比を1.5に制限します。
