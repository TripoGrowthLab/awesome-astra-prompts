...
```text
1. プロジェクトの目的
TITANIC — THE LAST LIGHTを制作します。船の最後の夕日から、衝突、避難、沈没、そして夜明けの追悼までを描く、264秒のインタラクティブなシネマティック航海です。来場者は演出された映像を鑑賞し、動く3D世界を探索し、チャプターへ移動して静止画を保存したり、映像全編をダウンロードしたりできます。法医学的な正確さや公式な関係を主張せず、芸術的な解釈として提示してください。

2. ビジュアル方針
抑制の効いたシネマティックなカラーパレットを使用します。深い大西洋の青を背景に、暖かいクリーム色と琥珀色の船内灯を配し、その後は星明かりの暗い夜から、涼しげな夜明けへ移行させます。2.39:1の映画的な構図によるパースペクティブシーンを、柔らかなブルーム、控えめな粒子感、ビネット付きでレンダリングします。キャラクターのポートレートと夜明けには被写界深度を使い、救難ロケットのパーティクルはシャープに保ちます。海のベースカラーは深い青とし、ワールド空間のうねり、小さめのミップマップ化した波紋ノーマル、フレネル反射を使います。暖かな夕日の色は主に反射光に適用します。同じ海面変位に追従する、船体に取り付けた航跡リボンを使用し、端は柔らかく、泡は途切れた状態にします。パノラマテクスチャは、水平方向にラップさせます。縦方向の空の継ぎ目や反射の筋を防ぐため、fractによる不連続が生じないようにしてください。オレンジ色の浅瀬シェーディング、小さく均一な波紋、発光する円形の泡デカールは避けます。奥行きの競合を防ぐため、見えない屋根の重なった上面を削除し、ショット距離に適したカメラのニアクリップ面を設定します。沈没中の窓明かりは、高周波のちらつきを加えず、単調に暗くしていきます。タイトルは静かなセリフ体とし、操作系は英語・中国語のバイリンガルにします。画面下部には細いタイムラインを配置します。

3. ワールド、地理、カメラ編集
全体で一貫した座標系を使用し、全長269メートルの船を配置します。船首は+X方向に向け、氷山は(275, 0, 57)に固定します。船は前進し、96.727秒で氷山に接触し、惰性で進んだ後に停止し、船首側と船尾側に分かれて沈没します。エンディングまで氷山を残し、夜明けの構図でも見えるようにします。0、63、110、163、211、241秒から始まる6つのチャプターを維持します。

意図的に構成した23ショットを制作します。船首での抱擁は29〜61秒にわたって描きます。導入用のアプローチショット、2人のクローズアップポートレート、海に向かう背後からの視点、斜めからのポートレートで構成します。船首先端で、Roseを前に、Jackをそのすぐ後ろに立たせ、2人とも船首の外側を見つめるようにします。シーケンス全体で夕日のライティングを維持します。127〜158秒には、救命ボートの実際のワールドトランスフォームにアタッチした3ショットを使用します。ボートデッキからの出発、乗客と吊り下げロープを捉えるより近い視点、水面への接近です。その後に、広角の避難、船体の傾斜、亀裂、沈没のショットを続けます。夜明けには、遠くに氷山を望む生存者のボートを見せ、その後に抑制の効いた追悼タイトルを表示します。

4. アセット一覧
- titanic-vessel：Blenderで全長269メートルの主要構造を制作します。連続したウェルデッキ、閉じた船首楼、段状のプロムナード、内部が空洞の傾斜したバフ色ファネル4本、黒い上部船体、赤い下部船体を備えます。864個の舷窓には実際の円形リムを、360個の窓にはフレームを使用します。エミッションは実際のガラス材に限定します。マテリアルをバッチ化し、沈没用にx=-32で分割します。マスト、リギング、ダビット、フォール、青銅製プロペラ、舵を追加します。チーク材のドアと真鍮のディテールを備えたP2のコンパニオンウェイを生成し、正規化したうえで、制作したデッキ上に2回再利用します。GLBを組み立てる際は、ノードのトランスフォームを保持します。
- atlantic-iceberg：層状の霜、変化のあるラフネス、控えめなノーマルマップ、説得力のある喫水線を備えた、不規則で浸食された青白い氷山を1つ制作します。地理的に固定されたオブジェクトとして扱います。
- lifeboat：白い木製の船体、暗いガンネル、ベンチ、オールを備えたWhite Starの手漕ぎ救命ボートを1隻制作し、独立して動く16隻のボートにインスタンス化します。
- bow-embrace：指定された1997年の映画の衣装とポーズに着想を得た、分離型の2人用キャラクターアセットです。Roseは赤褐色の髪、ネイビーとアイボリーの服、柄入りのショールを身に着け、腕を伸ばします。Jackはそのすぐ後ろに立ち、ダークコートとアイボリーのシャツを着ます。3D化する前に、クリーンなリファレンス画像を生成します。ヒーローキャラクターごとに、クリーンな頭部・首・肩のリファレンスから、クローズアップ用の頭部、首全体、上胸部を1つの連続したH v3.1サーフェスとして制作します。P2の抱擁ポーズ、手、衣装を維持します。クアッドリトポロジー、Blender上での局所的なサーフェス修正、4Kのカラー／ノーマルベイクを使用します。顎の直下に直接つなげるのではなく、衣服の開口部の下にインターフェースを合わせます。生成された連続した首を保持します。元の衣服の下にある、計測した胴体の輪郭に合わせて下胸部を調整し、連続したP2の上背部を復元し、不要な古い髪の残骸を削除します。浮いた襟パネルや、UVが引き伸ばされた首の押し出しは避けます。低い斜め方向の両側面と背面から、光が漏れる隙間や切断面の露出がないか確認します。制作したカメラビューから、まぶた、虹彩、唇、手、テクスチャの継ぎ目を確認します。布のシェーディングは、肌のシェーディングとは分けて、浅い入射角でのハイライトを抑えます。表情は自然なベース表情とし、体と布の動きは控えめにします。実際に実装していない限り、フェイシャルアニメーション用のリグがあるように見せないでください。
- seated-woman and seated-man：1912年の衣服と淡いコルク製ライフジャケットを身に着け、膝を曲げ、手を膝の上に置いて座る、成人乗客の個別モデルです。ボート間でジオメトリとマテリアルを共有し、配置と向きには少し変化をつけます。乗船用に、ボートごとのインスタンス数を切り替え可能にします。

主要な船にはBlenderを、デッキのコンパニオンウェイ、氷山、ボート、衣装、乗客にはTripo P2.0を、2体の連続したヒーローポートレートにはH v3.1を使用します。海、空のブレンド、星、ライティング、煙、救難ロケット、泡、飛沫、瓦礫はシーンエフェクトとして保持します。圧縮テクスチャを使用した軽量なWeb用モデルのバリエーションを用意し、オフラインでの映像書き出し用にフル解像度のアセットを保持します。

5. 再生とフィードバック
船、ボート、空、海面ノーマルに必要なアセットについて、実際の読み込み進捗を表示します。最初のシーンの準備ができた時点で開始ボタンを有効にし、その他のモデルと音楽の読み込みは遅延させます。必要なキャラクターや氷山のモデルが遅れている場合は、そのショットを黙ってスキップせず、シーンの境界で待機して準備ができ次第再開します。音声はユーザー操作後に開始します。

タイムラインは、前後方向のシークと、ゼロにリセットされない高速ドラッグに対応させます。シーク中も再生／一時停止とミュートの状態を保持し、古いオーディオクロックが要求された位置を上書きしないようにします。MP3とMP4にはバイト範囲リクエストを提供します。チャプターナビゲーションには、29秒の船首での抱擁と127秒の救命ボート降下への直接エントリーを含めます。これらを選択すると、再生状態を維持したまま監督用カメラを復元します。

探索中も、ワールド、船、サウンドトラックを動かし続けながら、オービット、ドラッグ、ズームを可能にします。視線方向をスナップさせず、船の移動に追従させます。一時停止は独立して機能し、映像に戻った際は現在の時刻を保持します。Spaceで再生／一時停止、矢印キーで10秒移動、Mでサウンド切り替え、Eで探索切り替え、Fでフルスクリーンを開きます。タッチによるオービット／ピンチと、タイムラインのタップにも対応します。

ボートは空の状態で開始します。112秒後から乗客が段階的なグループで乗り込み、各ボートが降下する前に乗船を完了します。ロープは、動くダビットと実際のボートの取り付け位置との間に張り、切り離し後に消去します。逆方向にシークした場合は、以前の乗客数とロープの状態を復元します。衝突では、船体／カメラの振動、氷片、擦過する飛沫、鋼鉄と氷の接触音の一過性音を連動させます。救難ロケットには、燃焼する白い星、短く個別の軌跡、重力、抗力、徐々に消える煙を使用します。沈没による乱流は、不規則で波に追従し、徐々に減衰するパッチとして表現します。飛沫は船尾の実際の喫水線に沿って分布させ、遠く離れた一点の噴水から発生させないでください。

6. 技術実装と納品物
Vite、JavaScriptモジュール、Three.jsを使用し、時間ベースで決定論的にアニメーションさせます。カメラ／タイムライン、船のアセット、キャラクター、環境、エフェクト、シーンの準備状態を分離します。Web再生、シーク、オフラインキャプチャで同じ時間モデルを共有します。Webレンダリングは、ピクセル、反射、シャドウの明示的な予算内に収め、より負荷の高いアンビエントオクルージョンはオフラインプロファイルに回します。長時間ブロックする起動時のプリウォームなしで、アセットのコンパイルとデコードを行います。スクリプト、モデル、画像、フォント、音声は同一オリジンでホストし、静的ビルドに秘密情報を含めないでください。

オリジナルスコアと、有料のElevenLabs Foleyを使用します。ElevenLabs Foleyには、完全な自然な口笛花火のテイクを実際の空中炸裂の位置で分割した、上昇飛行音と鋭い破裂音（余韻のある crackle 付き）、鋼鉄と氷の接触音および擦過音、救命ボートのロープと水の接触音、船体の応力／破断音、船尾による水の変位音を含めます。ソースWAVファイルを書き出し、プロンプトと履歴IDを保持し、タイミングに合わせたキューへ編集します。発射は119、151、183秒に合わせ、空中炸裂はそれぞれ3.15秒後に設定します。元の破裂音のアタックを保持し、オーケストラスコアは短時間だけダッキングします。ミックスの下には、静かな海、風、エンジンのアンビエンスを残します。公開Webサイトとダウンロード可能な映像の両方で、許可なく元の映画サウンドトラックを使用しないでください。フォールバックをサービス生成アセットとして説明するのではなく、実際のアセットと音源の出所を記録します。

3840×2160、24 fps、時間方向のサンプル3つで、6,336フレームを決定論的に書き出します。英語タイトルを焼き込み、2.39:1のレターボックス映像にします。4K H.264/AACマスターと、100 MiB未満の1080p Web版をエンコードします。どちらも再生時間は264秒、音声は48 kHzステレオとします。中国語／英語字幕、音声マスター、編集可能なソースはオプションとして保持します。プラットフォームアプリやページごとのWorkerを追加せず、既存のCMS Web Pagesホスティングを通じて静的ビルドを公開します。

7. 受け入れ基準
オープニング、2人のキャラクターポートレート、衝突、救難ロケット、3つすべての降下ショット、亀裂、船尾の消失、夜明けを確認します。244秒時点で氷山が消えないこと、海が深い青として見えること、沈没時に規則的な白い輪が現れないこと、制作済みのショットにキャラクターが存在し続けること、降下中もボートのロープと乗客が一貫して位置合わせされていることを確認します。モデルの遅延読み込み、前後方向のシーク、高速スクラブ、一時停止／ミュート、動的探索、クローズアップへの直接エントリー、タッチエミュレーションをテストします。書き出したすべてのフレームを検証し、両方の映像を完全にデコードし、実際のブラウザからのダウンロードと納品ファイルを比較し、公開ビルドとCMSの関連付けを確認します。ブラウザのモバイルエミュレーションと、実機のスマートフォンでのテストは区別してください。
