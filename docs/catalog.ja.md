...
<a href="https://www.tripo3d.ai/ja/3d-prompts/bubble-bay"><img src="../assets/previews/5158051d1bd3d80bc65cbfdd00a31c6e9c048c21f0e18ed7b1826972c94b6b5d.webp" width="840" loading="lazy" alt="バブルベイ：3D水風船バトル"></a>

<a href="https://www.tripo3d.ai/ja/3d-prompts/bubble-bay"><img src="../assets/previews/ceda95626d7f2a404ab04a28ea483f2da5eb9cc367184a09da386305cc85539a.png" width="840" loading="lazy" alt="バブルベイ：3D水風船バトル"></a>

水風船、ジャンプ、AI対戦相手、自分で操作するアニメーションキャラクターが登場する、明るい3D中庭バトルを作成します。Tripoモデルと、Three.jsで作る幾何学バージョンを切り替えられます。

**プロンプト**

```text
1. プロジェクトの目的
明るいおもちゃの街の中庭を舞台に、プレイ可能な3D水風船バトル「Bubble Bay」を制作します。プレイヤーは青チームのキャラクター1体を操作し、赤チームのAI対戦相手2体と戦います。通路を走り、木箱を壊し、強化アイテムを集め、足場にジャンプし、対戦相手を泡に閉じ込めます。3分間の試合中に相手チームを3回捕獲すると勝利です。Three.jsで完成度の高いゲームを制作し、ビジュアルモードの比較表示と、リギング済みの自作キャラクターを試合に追加できるワークショップを用意します。

2. ビジュアル方針
添付された中庭のスクリーンショットとゲームプレイ録画を目標バージョンとして使用します。キャラクターを斜め上後方から滑らかに追従する、視野角約51度の近距離三人称カメラを作成します。プレイエリアが画面いっぱいに表示されるようにします。クリーム色の石畳、ハニーイエローの木箱、赤レンガの花壇、青とテラコッタ色のコテージ屋根、丸みを帯びた緑の木々、小さな花のまとまりを配置します。3体のキャラクターはシルエットを見分けやすくし、表面には柔らかな陰影とテクスチャを付けます。暖色系の指向性のある日光、ソフトな接地影、穏やかなフォグ、控えめなブルーム風の水面ハイライト、透明な青い泡を加えます。詳細なシーン上でも、スコア、タイマー、操作方法、対戦相手が読みやすい状態を保ちます。
ホーム画面には、左側にコンパクトなキャラクターセレクター、3つの能力値インジケーター、難易度選択、目立つスタートボタンを重ねて表示します。画面上部中央にはTripoとThree.jsのビジュアルモードを目立つように配置し、ラベル、選択状態のスタイル、表示モデルを連動して切り替えます。デフォルトはTripoモードにします。プレイ中は、チームスコア、タイマー、プレイヤーマーカー、現在の能力値、操作ヒントだけにインターフェースを絞ります。中央のリザルトパネルには、スコア、ベスト結果、リプレイ、キャラクターワークショップへのアクションを表示します。

3. ワールドとレイアウト
約22×24ワールドユニットの長方形の中庭を作り、セルの間隔は2ユニットにします。中央を南北に通る、見通しのよい通路と、南端付近の安全な開始エリアを確保します。つながったルートの周囲に、壊れない花壇と破壊可能な木箱を交互に配置します。周囲にはコテージ、木々、花を散らして配置しますが、プレイエリアを隠さないようにします。詳細な装飾メッシュとは独立した、シンプルなコリジョンボリュームを使用します。花壇の支持高さは1.02ユニット、木箱は1.72ユニットにします。キャラクターが実際に上面へ着地し、端から歩いて出ると落下するようにします。木箱は破壊されると消え、新しいルートが開きます。プレイヤーは南側付近、対戦相手2体は北側半分にスポーンさせます。最初から到達できる強化アイテムを用意し、試合ごとに木箱の配置を変えます。

4. アセット一覧
境界、向き、接地ピボットを統一した、個別に差し替え可能なGLBスロットを使用します。キャラクターを最優先し、次に繰り返し配置する木箱とコテージ、その後に植生を優先します。
- hero：丸みを帯びた頭部とコンパクトな体を持つ、デフォルトの青いカートゥーン風冒険者「Blue」。リギング済みキャラクターアセットを1体使用し、デフォルトでは青チームのプレイヤーにします。待機、走行、ジャンプに対応させます。
- mint：ピンク色の頭部アクセントがある、小さなターコイズ色のファンタジークリーチャー「Mint」。リギング済みキャラクターアセットを1体使用し、プレイヤーが選択できるほか、対戦相手にも使用できるようにします。待機と走行に対応させます。
- mango：クリーム色の顔とお腹を持つ、オレンジ色の動物風マスコット「Mango」。リギング済みキャラクターアセットを1体使用し、プレイヤーが選択できるほか、対戦相手にも使用できるようにします。待機と走行に対応させます。
- cottage：クリーム色の壁と、厚みのある瓦ぶきの切妻屋根を持つ小さなコテージ。境界に沿って繰り返し配置し、ジオメトリを再利用して、青またはテラコッタ色の屋根バリエーションを適用します。
- crate：板のディテールと頑丈なシルエットを持つ、ハニー色の木箱。破壊可能なセル全体に繰り返し配置し、壊れたときや異なる角度から見たときも、内部が閉じた状態に見えるようにします。
- tree：太い幹と、丸みを帯びた層状の葉を持つ、背の低いスタイライズドツリー。中庭の境界の外側に繰り返し配置します。
- bush：小さな色とりどりの花を含む、背の低い緑の植生。花壇の中や家の周囲に繰り返し配置します。
地面の舗装、花壇のレンガ、空、ライト、コリジョンプロキシ、泡、十字形の水しぶき、パーティクル、アイテム、すべてのインターフェース要素はプロシージャルにします。Three.jsモード用に、シーンとキャラクターの対応する幾何学バージョンを用意します。ビジュアルモードを切り替えても、試合の状態、位置、タイマー、スコアを維持します。

5. ゲームプレイとフィードバック
デルタタイムを使用した連続移動、斜め移動時の速度正規化、コリジョン、滑らかな追従カメラを実装します。キーボード操作は、WASDまたは矢印キーで移動、Fで泡を設置、Spaceでジャンプ、Q/Eでカメラを回転、シーンをドラッグしてカメラの向きとピッチを調整、Escで一時停止、Shiftで0.3秒間のダッシュを発動します。ダッシュのクールダウンは4秒にします。タッチ画面では、左側のジョイスティックと、右側に独立した泡ボタンおよびジャンプボタンを配置し、同時に操作できるようにします。
泡は地面のセルを2.5秒間占有し、その後、4方向のグリッドに沿って十字形の水しぶきを放ちます。固い花壇と境界が水しぶきを止めます。各方向で最初に木箱へ当たったものは壊れ、その水しぶきの腕を止めます。近くの泡は連鎖反応します。設置直後の泡から所有者が離れられるようにしつつ、その泡を通り抜けて戻れないようにします。アクティブな泡の上限を守り、空中にいる間は設置できないようにします。
水に触れるとキャラクターは3.5秒間閉じ込められます。Spaceを4回個別にタップすると脱出できます。Xを押すと、試合ごとに1本だけ使える救出ニードルを消費し、即座に脱出します。対戦相手のキャラクターが閉じ込められたキャラクターに触れると、即座に捕獲します。それ以外の場合は、拘束時間が切れた時点で相手チームに捕獲を与え、犠牲になったキャラクターをリスポーンさせます。赤チームの味方同士は捕獲もダメージも与えません。自分で閉じ込められた場合も、時間切れになると相手チームに捕獲を与えます。高い位置へのジャンプや足場の上面によって、低い位置を走る水しぶきを回避できます。
初期プロフィール：Blueは泡2個、射程2、移動速度4.25。Mintは泡1個、射程2、速度5.15。Mangoは泡1個、射程3、速度4.55です。泡、射程、速度の強化アイテムによって、対応する能力値を向上させます。アイテム取得時のフィードバックを読みやすく表示し、水、移動、ジャンプ、拘束、捕獲のエフェクトには素早く反応する操作感を持たせます。短い合成効果音を追加し、ミュート操作を用意します。
先に3回捕獲したチームが勝利します。180秒経過時点でチームスコアを比較し、同点の場合は引き分けにします。移動、危険回避、泡の設置、捕獲の追跡を行う、イージーとノーマルの対戦相手を用意します。一時停止中はシミュレーションを停止し、リプレイでは試合をリセットします。ベスト結果はローカルに保存します。

6. 技術実装とキャラクターワークショップ
Three.js 0.180、JavaScript ESモジュール、Viteを使用します。シミュレーション、シーン描画、モデル読み込み、入力・インターフェース、翻訳を分離します。ランタイム依存関係、GLB、テクスチャ、通知は同一オリジンの静的ファイルにまとめ、プレイにアカウントやサーバーを必要としないようにします。リギング済みモデルにはGLTFLoader、SkeletonUtilsによるクローン、AnimationMixerを使用し、繰り返し配置する環境アセットにはジオメトリとマテリアルをプールまたは再利用します。納品するプロジェクトでは、Tripo P2で生成したキャラクターおよび環境アセットを使用します。出典情報を保持し、スケルトンと埋め込みアニメーションが機能する状態を維持します。
中国語、英語、韓国語のインターフェースを用意します。保存された有効な手動言語選択が常に優先されます。それ以外の場合は、デバイスのIANAタイムゾーンを使用します。Asia/ShanghaiやAsia/Urumqiなど中国本土のタイムゾーンでは中国語、Asia/SeoulやAsia/Pyongyangでは韓国語、香港、マカオ、台湾、およびその他または利用できないタイムゾーンでは英語を選択します。従来のエイリアスも一貫して処理します。UTCオフセットやブラウザ言語から中国本土を推測しないでください。自動選択の結果を手動設定として保存しないでください。
ワークショップでは、テクスチャ埋め込み済みで最大40 MBのGLBを受け付け、キャラクターを追加する前に実際のスキニング、スケルトン、ジョイント、ウェイトを検証します。リギングされていない静的モデルは、明確な説明を添えて拒否します。モデルはブラウザ内でローカルに解析します。ファイル内容のハッシュから安定した初期プロフィールを生成し、同じファイルには同じバランスの能力値が割り当てられるようにします。90度の向き調整を用意します。
リギングとスキニングは必須ですが、対応する標準ヒューマノイドスケルトンでは、個別のアニメーションクリップは任意であることを説明します。埋め込み済みの待機、走行、歩行、ジャンプのクリップを優先し、それ以外の場合は認識可能な手足のボーンを使って基本的な待機と歩行を動かします。認識できないリグや非ヒューマノイドリグには、使用可能なアニメーションクリップを含める必要があります。任意のスケルトンに対して自動アニメーションを保証しないでください。非対応の圧縮GLB形式と、標準の非圧縮ファイルとして書き出す方法を説明します。アップロードしたキャラクターは、更新後に再度選択する必要があります。
https://studio.tripo3d.ai/?utm_source=satellite_invite&utm_medium=bubble-bay&utm_campaign=create-characterへリンクする、わかりやすいキャラクター作成アクションを用意します。作成、リギング、書き出し、ゲームへの復帰手順を説明します。このヘルプはワークショップ内に置き、最初の試合を妨げないようにします。

7. 受け入れ条件と納品
編集可能なソース、依存関係のロックファイル、README、ローカル実行手順、本番用静的ビルド、アクセス可能なプレイURLを納品します。ホーム画面、試合中、拘束状態、水しぶき、足場への着地、リザルト、一時停止、リプレイを検証します。ゲームプレイをリセットせずに、TripoとThree.jsの違いが目で確認できることを確認します。実際のキーボード入力とタッチエミュレーションを試験し、移動と泡の設置を同時に行えることも確認します。エミュレーションであることを正直に表示し、実機での動作確認を行ったとは主張しません。キャラクターのインポートでは、リギングなしのモデルが拒否されること、アニメーション付きリグ、クリップなしでも対応するリグをテストします。中国本土、香港、マカオ、台湾、韓国、その他のタイムゾーンと、手動選択の保存を確認します。実際にホストされたページ、同一オリジンのアセット、ワークショップリンク、埋め込み利用を検証します。納品された中庭バージョンと一致する実際のゲームプレイ録画と代表的なスクリーンショットを保持し、以前の遠景の島レイアウトに置き換えないでください。作者・出典表示と第三者ライセンスの通知を保持し、参照サイトのアセットは再配布しません。

```

[詳細を見る ↗](https://www.tripo3d.ai/ja/3d-prompts/bubble-bay) · [デモ](https://bubble-bay.tripo.page/) · [作例一覧に戻る](#all-prompts)
...
<tr><td align="center">
<br>
<p><strong><a href="https://www.tripo3d.ai/ja/3d-prompts/models/gpt-6-astra?utm_source=github&amp;utm_medium=referral&amp;utm_campaign=awesome_astra_prompts&amp;utm_content=catalog_bottom">全 264 件の作例を公式サイトで見る →</a></strong></p>
