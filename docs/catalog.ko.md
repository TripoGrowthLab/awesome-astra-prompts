...
- [인터랙티브 복셀 나이트 모닥불 장면](#2100350159540596760)
...
<a href="https://www.tripo3d.ai/ko/3d-prompts/gpt-6-astra-2100350159540596760"><img src="../assets/previews/c86d545b161a6223e73f07626532a18421dad108dd5d879ee1e9d26e60cb1329.jpg" width="840" loading="lazy" alt="인터랙티브 복셀 나이트 모닥불 장면"></a>

절차적으로 생성한 나이트, 모닥불, 선택 가능한 나이트 행동, 카메라 프리셋, 레트로 HUD, 합성 앰비언트 오디오를 포함한 인터랙티브 Three.js 복셀 스타일 밤의 숲 캠프 장면입니다. 게시 작성자는 이 프롬프트와 이미지를 GPT-6 Astra에 제공했다고 밝혔습니다.
...
<a href="https://www.tripo3d.ai/ko/3d-prompts/gpt-6-astra-2100838090210431302"><img src="../assets/previews/beed044dc7075346b2830bb1c618e8b8f142481efb63cc04db1116a2955d521a.jpg" width="840" loading="lazy" alt="하늘섬 사이를 오가는 3D 공중 트램 게임"></a>

인증된 작성자의 댓글에 원글에서 GPT-6와 Gemini 4에 실행했다고 밝힌 전체 프롬프트가 공유되어 있습니다. 구름 바다와 지중해풍 섬을 배경으로, 조작 기능과 승객 편안함 점수, 정거장, 업그레이드, 모바일 지원을 갖춘 브라우저용 Three.js 공중 트램 게임을 제작해 달라는 내용입니다.
...
<a href="https://www.tripo3d.ai/ko/3d-prompts/gpt-6-astra-2100844566718926949"><img src="../assets/previews/f8596f35d968eda4e24acabd4e2405d42aae7745029b8cf3426d550a2bd2cd52.jpg" width="840" loading="lazy" alt="실사풍 3D 월드 만들기"></a>

실사풍으로 구현한 탐험형 3D 월드를 한 번에 요청하는 루트 게시물의 제안 문구입니다. 이 게시물에서는 Blender MCP 워크플로의 일부로 소개하지만, 이 프롬프트가 함께 제공된 쇼케이스의 입력으로 사용되었다고 명시하지는 않습니다.
...
<a href="https://www.tripo3d.ai/ko/3d-prompts/gpt-6-astra-2100956517633761447"><img src="../assets/previews/d7437e20a189ef08ef24c9272fa803e0f9e8d7da7da70314a92980db6a752bdc.jpg" width="840" loading="lazy" alt="인터랙티브 IWC Schaffhausen 시계 모델"></a>

검증된 작성자의 댓글에 고도로 디테일한 Three.js 인터랙티브 3D IWC Schaffhausen 시계 제작 프롬프트가 포함되어 있습니다. 각 기계 부품을 개별적으로 식별할 수 있고, 사실적인 사파이어 글라스와 실시간 시곗바늘 움직임, 분해 및 재조립 인터랙션을 구현하도록 요구합니다. 원문 게시물에서는 GPT-6 Astra를 비롯한 여러 모델을 비교하는 공통 프롬프트로 소개합니다.
...
<a href="https://www.tripo3d.ai/ko/3d-prompts/bubble-bay"><img src="../assets/previews/5158051d1bd3d80bc65cbfdd00a31c6e9c048c21f0e18ed7b1826972c94b6b5d.webp" width="840" loading="lazy" alt="버블 베이: 3D 물풍선 배틀"></a>

<a href="https://www.tripo3d.ai/ko/3d-prompts/bubble-bay"><img src="../assets/previews/ceda95626d7f2a404ab04a28ea483f2da5eb9cc367184a09da386305cc85539a.png" width="840" loading="lazy" alt="버블 베이: 3D 물풍선 배틀"></a>

햇살이 비치는 3D 마을 광장에서 물풍선을 던지고, 점프하며, AI 상대와 대결하고, 직접 만든 애니메이션 캐릭터도 플레이할 수 있는 전투 게임을 만들어 보세요. Tripo 모델과 기하학적 Three.js 버전을 전환해 비교할 수 있습니다.

**프롬프트**

```text
1. 프로젝트 목표
햇살이 비치는 장난감 마을 광장에서 플레이 가능한 3D 물풍선 배틀 게임 ‘버블 베이’를 제작합니다. 플레이어는 파란 팀 캐릭터 하나를 조작해 빨간 팀 AI 상대 두 명과 대결합니다. 통로를 달리고, 상자를 부수고, 강화 아이템을 획득하고, 플랫폼으로 점프하고, 상대를 물방울에 가둘 수 있어야 합니다. 3분 경기 안에 상대 팀을 세 번 포획하면 승리합니다. 시각 모드 비교 화면이 제공되는 완성형 Three.js 게임과, 개인 리깅 캐릭터를 경기에 불러올 수 있는 캐릭터 작업실을 구현합니다.

2. 비주얼 방향
첨부된 광장 스크린샷과 게임플레이 녹화 영상을 목표 버전으로 삼습니다. 캐릭터를 위쪽과 뒤쪽에서 부드럽게 따라가는 약 51도 화각의 근접 3인칭 카메라를 구성합니다. 플레이 공간이 화면을 가득 채우도록 합니다. 크림색 석재 포장, 꿀빛 노란색 나무 상자, 붉은 벽돌 화분, 파란색과 테라코타색 지붕의 오두막, 둥근 형태의 초록 나무, 작은 꽃이 모인 군락을 사용합니다. 세 캐릭터의 실루엣이 명확하게 보이도록 하고, 표면에는 부드러운 셰이딩과 텍스처를 적용합니다. 따뜻한 방향성 햇빛, 부드러운 접촉 그림자, 은은한 안개, 절제된 블룸 느낌의 물 하이라이트와 투명한 파란 물방울을 추가합니다. 디테일한 장면 위에서도 점수, 타이머, 조작법과 상대가 잘 보이도록 합니다.
홈 화면에는 왼쪽에 간결한 캐릭터 선택 UI, 세 가지 능력치 표시, 난이도 선택지와 눈에 잘 띄는 시작 버튼을 배치합니다. 화면 상단 중앙에는 Tripo와 Three.js 비주얼 모드를 크게 표시하고, 모드 라벨과 선택 상태, 표시 모델이 함께 전환되도록 합니다. 기본 모드는 Tripo로 설정합니다. 플레이 중에는 팀 점수, 타이머, 플레이어 표시, 현재 능력치와 조작 힌트만 남겨 인터페이스를 간소화합니다. 점수, 최고 기록, 다시 플레이하기와 캐릭터 작업실 동작을 포함한 결과 패널을 화면 중앙에 표시합니다.

3. 월드와 레이아웃
약 22×24 월드 유닛 크기의 직사각형 광장을 만들고, 셀 간격은 2유닛으로 설정합니다. 중앙을 남북으로 가로지르는 통로를 비워 두고, 남쪽 가장자리 근처에 안전한 시작 구역을 둡니다. 연결된 경로를 따라 고정형 화분과 파괴 가능한 상자를 번갈아 배치합니다. 광장 외곽에는 오두막, 나무, 꽃을 흩어 배치하되 플레이 영역을 가리지 않도록 합니다. 상세한 장식 메시와 충돌 볼륨은 서로 독립적으로 구성합니다. 화분의 지지 높이는 1.02유닛, 상자의 높이는 1.72유닛으로 설정합니다. 캐릭터가 실제로 상자와 화분 위에 착지하고, 가장자리에서 걸어 나오면 떨어지도록 합니다. 상자가 파괴되면 사라져 새로운 경로가 열립니다. 플레이어는 남쪽 근처에, 상대 두 명은 북쪽 절반에 생성합니다. 처음부터 도달할 수 있는 강화 아이템을 제공하고, 경기마다 상자 배치를 바꿉니다.

4. 에셋 목록
일관된 바운드, 방향과 지면 피벗을 사용하는 교체 가능한 독립 GLB 슬롯을 구성합니다. 캐릭터를 우선하고, 그다음 반복 배치되는 상자와 오두막, 마지막으로 식생을 우선순위에 둡니다.
- hero: 둥근 머리와 아담한 체형을 가진 기본 파란색 카툰 모험가 Blue입니다. 리깅된 캐릭터 에셋 하나를 사용하며, 기본적으로 파란 팀 플레이어로 지정합니다. 대기, 달리기, 점프 애니메이션을 지원합니다.
- mint: 분홍색 머리 장식이 있는 작은 청록색 판타지 생물 Mint입니다. 리깅된 캐릭터 에셋 하나를 사용하며, 플레이어가 선택하거나 상대 캐릭터로 사용할 수 있습니다. 대기와 달리기 애니메이션을 지원합니다.
- mango: 크림색 얼굴과 배를 가진 주황색 동물형 마스코트 Mango입니다. 리깅된 캐릭터 에셋 하나를 사용하며, 플레이어가 선택하거나 상대 캐릭터로 사용할 수 있습니다. 대기와 달리기 애니메이션을 지원합니다.
- cottage: 크림색 벽과 큼직한 기와 박공지붕으로 구성된 작은 오두막입니다. 경계선을 따라 반복 배치하고, 지오메트리를 재사용하면서 파란색 또는 테라코타색 지붕 변형을 적용합니다.
- crate: 판자 디테일과 견고한 실루엣을 가진 꿀빛 나무 상자입니다. 파괴 가능한 셀 전체에 반복 배치하고, 부서졌을 때나 다른 각도에서 보았을 때도 내부가 비어 보이지 않도록 구성합니다.
- tree: 굵은 줄기와 둥글게 층을 이룬 잎을 가진 짧은 스타일라이즈드 나무입니다. 광장 경계 뒤쪽에 반복 배치합니다.
- bush: 작고 다채로운 꽃이 있는 낮은 초록색 식생입니다. 화분 안과 집 주변에 반복 배치합니다.
바닥 포장, 화분 벽돌, 하늘, 조명, 충돌 프록시, 물방울, 십자형 물 폭발, 파티클, 픽업 아이템과 모든 인터페이스 요소는 절차적으로 생성합니다. Three.js 모드에서 사용할 장면과 캐릭터의 기하학적 버전도 함께 제공합니다. 비주얼 모드를 바꿔도 경기 상태, 위치, 타이머와 점수는 유지되어야 합니다.

5. 게임플레이와 피드백
델타 타임 기반의 연속 이동, 정규화된 대각선 속도, 충돌 처리와 부드러운 추적 카메라를 사용합니다. 키보드 조작: WASD 또는 방향키로 이동, F로 물방울 설치, Space로 점프, Q/E로 카메라 회전, 장면을 드래그해 카메라 방향과 피치 조정, Esc로 일시정지, Shift로 0.3초간 대시를 실행합니다. 대시 쿨다운은 4초입니다. 터치 화면에서는 왼쪽 조이스틱과 오른쪽의 물방울 및 점프 버튼을 별도로 제공하며, 두 버튼을 동시에 사용할 수 있도록 합니다.
물방울은 2.5초 동안 바닥 셀을 차지한 뒤, 네 방향의 격자 경로를 따라 십자형 물 폭발을 일으킵니다. 고정형 화분과 경계는 물 폭발을 막습니다. 각 방향에서 처음 맞은 상자는 부서지면서 해당 폭발 가지를 멈춥니다. 가까이 있는 물방울은 연쇄 반응을 일으킵니다. 설치 직후에는 물방울의 주인이 빠져나갈 수 있게 하되, 그 물방울을 통과해 되돌아가지는 못하게 합니다. 설치 가능한 활성 물방울 수 제한을 적용하고, 공중에 떠 있는 동안에는 설치할 수 없도록 합니다.
물에 맞은 캐릭터는 3.5초 동안 갇힙니다. Space를 네 번 따로 누르면 탈출할 수 있으며, X를 누르면 경기당 한 번 사용할 수 있는 구조 바늘 하나를 소모해 즉시 탈출합니다. 상대 캐릭터가 갇힌 캐릭터에 닿으면 즉시 포획합니다. 그렇지 않으면 덫의 제한 시간이 끝날 때 상대 팀에 포획 1회를 부여하고 피해자를 다시 생성합니다. 같은 빨간 팀 캐릭터끼리는 서로 포획하거나 피해를 주지 않습니다. 자신이 설치한 물방울에 갇힌 경우에도 제한 시간이 끝나면 상대 팀에 포획 1회를 부여합니다. 높은 점프와 플랫폼 위에서는 낮은 물 폭발을 피할 수 있습니다.
초기 능력치: Blue는 물방울 2개, 사거리 2, 이동 속도 4.25입니다. Mint는 물방울 1개, 사거리 2, 속도 5.15입니다. Mango는 물방울 1개, 사거리 3, 속도 4.55입니다. 물방울, 사거리, 속도 픽업 아이템은 해당 능력치를 향상합니다. 픽업 획득, 물 효과, 이동, 점프, 포획과 구속 효과를 명확하고 즉각적으로 보여 줍니다. 짧은 합성 효과음을 추가하고 음소거 기능을 제공합니다.
먼저 3회를 포획한 팀이 승리합니다. 180초가 되면 팀 점수를 비교하고, 동점이면 무승부로 처리합니다. 이동하고, 위험을 피하고, 물방울을 설치하고, 포획을 시도하는 쉬움 및 보통 난이도의 상대를 포함합니다. 일시정지 중에는 시뮬레이션이 멈추고, 다시 플레이하기를 선택하면 경기가 초기화됩니다. 최고 기록은 로컬에 저장합니다.

6. 기술 구현과 캐릭터 작업실
Three.js 0.180, JavaScript ES 모듈과 Vite를 사용합니다. 시뮬레이션, 장면 렌더링, 모델 로딩, 입력/인터페이스와 번역을 분리합니다. 런타임 의존성, GLB, 텍스처와 안내 문구는 동일 출처의 정적 파일로 번들링하며, 플레이에 계정이나 서버가 필요하지 않도록 합니다. 리깅 모델에는 GLTFLoader, SkeletonUtils 복제와 AnimationMixer를 사용하고, 반복되는 환경 에셋에는 풀링하거나 재사용하는 지오메트리와 머티리얼을 적용합니다. 완성된 프로젝트는 Tripo P2로 생성한 캐릭터 및 환경 에셋을 사용합니다. 에셋의 출처를 유지하고 스켈레톤과 내장 애니메이션이 정상적으로 작동하도록 합니다.
중국어, 영어와 한국어 인터페이스를 제공합니다. 유효하게 저장된 수동 언어 선택은 항상 우선합니다. 그렇지 않으면 기기의 IANA 시간대를 사용합니다. Asia/Shanghai, Asia/Urumqi 같은 중국 본토 시간대는 중국어를 선택하고, Asia/Seoul 및 Asia/Pyongyang은 한국어를 선택합니다. 홍콩, 마카오, 대만과 그 밖의 시간대 또는 확인할 수 없는 시간대는 영어를 선택합니다. 레거시 별칭도 일관되게 처리합니다. UTC 오프셋이나 브라우저 언어만으로 중국 본토를 추론하지 않습니다. 자동 선택 결과를 수동 설정으로 저장하지 않습니다.
내장 텍스처가 포함된 최대 40MB의 GLB를 작업실에서 받을 수 있도록 하고, 캐릭터를 추가하기 전에 실제 스키닝, 스켈레톤, 조인트와 웨이트를 검증합니다. 리깅되지 않은 정적 모델은 명확한 설명과 함께 거부합니다. 모델은 브라우저에서 로컬로 파싱합니다. 파일 콘텐츠 해시를 기반으로 안정적인 초기 프로필을 산출하여, 같은 파일에는 항상 동일한 균형형 능력치를 부여합니다. 방향을 90도 조정하는 기능을 제공합니다.
지원되는 표준 휴머노이드 스켈레톤에서는 리깅과 스키닝이 필수이고, 별도의 애니메이션 클립은 선택 사항임을 설명합니다. 내장된 대기/달리기/걷기/점프 클립을 우선 사용하고, 클립이 없으면 식별 가능한 사지 본을 사용해 기본 대기와 걷기를 구현합니다. 인식할 수 없거나 비휴머노이드 리그에는 사용할 수 있는 애니메이션 클립이 포함되어야 합니다. 임의의 스켈레톤에 자동 애니메이션을 제공한다고 약속하지 않습니다. 지원되지 않는 압축 GLB 형식과 표준 비압축 파일로 내보내는 방법을 설명합니다. 업로드한 캐릭터는 새로고침 후 다시 선택해야 합니다.
https://studio.tripo3d.ai/?utm_source=satellite_invite&utm_medium=bubble-bay&utm_campaign=create-character로 연결되는 명확한 캐릭터 생성 동작을 제공합니다. 생성, 리깅, 내보내기와 게임으로 돌아오는 단계를 설명합니다. 첫 경기를 막지 말고 이 도움말을 작업실 안에 배치합니다.

7. 검수 및 납품
편집 가능한 소스, 의존성 잠금 파일, README, 로컬 실행 안내, 프로덕션용 정적 빌드와 접근 가능한 플레이 URL을 제공합니다. 홈 화면, 실제 전투, 구속 상태, 물 폭발, 플랫폼 착지, 결과, 일시정지와 다시 플레이하기를 검증합니다. 게임플레이를 초기화하지 않고 Tripo/Three.js 모드 간 차이가 눈에 보이는지 확인합니다. 실제 키보드 입력과 터치 에뮬레이션을 테스트하고, 이동과 물방울 설치를 동시에 사용하는 경우도 포함합니다. 에뮬레이션임을 정확히 표시하고 실제 기기 지원을 검증했다고 주장하지 않습니다. 리깅되지 않은 모델의 거부, 애니메이션이 있는 리그와 클립이 없어도 지원되는 리그를 각각 테스트합니다. 중국 본토/홍콩/마카오/대만/한국/기타 시간대와 수동 선택 저장을 확인합니다. 실제 호스팅 페이지, 동일 출처 에셋, 작업실 링크와 임베드 사용을 검증합니다. 완성된 광장 버전과 일치하는 실제 게임플레이 녹화와 대표 스크린샷을 보관하며, 이전의 멀리 떨어진 섬 레이아웃으로 대체하지 않습니다. 작성자 및 소스 표기와 서드파티 라이선스 고지를 유지하되, 참고 사이트의 에셋은 재배포하지 않습니다.

```

[자세히 보기 ↗](https://www.tripo3d.ai/ko/3d-prompts/bubble-bay) · [데모](https://bubble-bay.tripo.page/) · [사례 목록으로](#all-prompts)
...
<a href="https://www.tripo3d.ai/ko/3d-prompts/titanic-the-last-light"><img src="../assets/previews/804ff98b5bbd18f23ba66fe1a98ef14b46381b11adb913f606281899db4ecbc0.jpg" width="840" loading="lazy" alt="타이타닉 — 마지막 빛"></a>

짙푸른 대서양을 배경으로 인물 초상, 실제 변환을 추적한 구명정 하강, 탐험할 수 있는 움직이는 세계를 담아낸 4분 24초 분량의 시네마틱 타이타닉 항해입니다.
...
<tr><td align="center">
<br>
<p><strong><a href="https://www.tripo3d.ai/ko/3d-prompts/models/gpt-6-astra?utm_source=github&amp;utm_medium=referral&amp;utm_campaign=awesome_astra_prompts&amp;utm_content=catalog_bottom">공식 사이트에서 전체 사례 264개 보기 →</a></strong></p>
