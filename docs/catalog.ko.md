...
<a href="https://www.tripo3d.ai/ko/3d-prompts/bubble-bay"><img src="../assets/previews/3f9d3b8fcab4a54ff4bf19c6f25eb3f3ce6ce697cb929ab0abf86e89d6b8f708.jpg" width="840" loading="lazy" alt="버블 베이: 3D 물풍선 배틀"></a>

<a href="https://www.tripo3d.ai/ko/3d-prompts/bubble-bay"><img src="../assets/previews/a0927800e707bb993467cd9c9d32d7955cafcad0fe1ab3986c33d6bec6ce85f8.jpg" width="840" loading="lazy" alt="버블 베이: 3D 물풍선 배틀"></a>

<a href="https://www.tripo3d.ai/ko/3d-prompts/bubble-bay"><img src="../assets/previews/44f0d05c544d89c5044aece0e9da8cc3730bc51d81f395ff0e554bbceb4f2ab2.jpg" width="840" loading="lazy" alt="버블 베이: 3D 물풍선 배틀"></a>

Pirate14와 Village10에서 애니메이션이 적용된 P2 캐릭터 3명으로 클래식 3D 물풍선 배틀을 제작하세요. 근접 시점, 무지갯빛 트랩 버블, 설정 가능한 포획 목표, AI 라이벌, 커스텀 캐릭터 임포트와 오리지널 ElevenLabs 음악을 포함합니다. 포획 목표는 기본 6회로 설정하고, 로딩이 완료된 후에만 비교 탭을 표시하세요.

**프롬프트**

```text
클래식 BnB를 연상시키는 플레이 가능한 Three.js 물풍선 아레나 Bubble Bay를 제작하세요. 기본값은 디테일한 Tripo 모델로 설정하고, 매치를 유지한 채 Three.js 지오메트리와 비교할 수 있는 전환 스위치를 눈에 잘 띄게 제공하세요. Boz/Bazzi, Ike/Uni, Plunk/Keffy에 대해 제공된 오리지널 캐릭터 레퍼런스를 사용하고, 막연히 비슷한 텍스트 생성 디자인으로 대체하지 말고 얼굴 특징, 모자, 색상, 의상과 신체 비율을 유지하세요.

캐릭터와 픽업 지오메트리는 명시적으로 tripo-p2를 지정한 Tripo CLI로 생성하세요. 정면을 향한 중립 레퍼런스 포즈를 준비하고, 모델을 생성한 뒤 유효한 스킨 웨이트가 적용된 실제 이족보행 스켈레톤을 바인딩하여 GLB로 내보내세요. 태스크 ID, 크레딧, 원본 스키닝 파일과 최종 해시를 보존하세요. 대기, 달리기, 점프 애니메이션은 실제 관절을 움직여야 합니다. 과도하게 큰 머리, 귀, 장갑의 스키닝을 수정하고 여러 시점과 동작을 점검하세요. 로컬에서 제작한 클립은 Tripo 프리셋인 것처럼 표시하지 말고 정확하게 설명하세요. 오리지널 캐릭터의 공개 사용 권리는 별도로 확인해야 하며, AI 생성은 사용 허가가 아닙니다.

현지화된 이름: 宝宝 / 배찌 / Boz (Bazzi), 小乖 / 우니 / Ike (Uni), 胖墩 / 케피 / Plunk (Keffy). 시작 수용량/사거리/속도 레벨: 1/1/5, 1/2/5, 1/2/4. 최대치: 6/7/9, 6/7/8, 9/8/8. 속도는 0.25 + 레벨*0.8 월드 단위/초로 변환하고, 타일 크기는 2 단위로 설정하세요. 캐릭터를 선택하면 나머지 두 캐릭터를 서로 다른 라이벌로 지정하고, 각 캐릭터의 프로필에 맞는 능력치를 적용하세요.

기본 맵으로 15x13 Pirate/Patrit14와 Village10을 제공하세요. 식별 가능한 금색 갑판, 노란 화물, 나무 상자, 대포 4문과 중앙 돛대를 유지하세요. 마을에는 색상별 주거 지구 4곳, 중앙 도로, 생울타리와 블록 장난감이 있어야 합니다. 퍼블리셔 맵을 레퍼런스로 사용하되 런타임 아트워크는 직접 구성하고, 끊김 없는 3D 이동과 AI 탈출에 필요한 작은 경로 개방부를 문서화하세요. 밝은 머티리얼, 그림자, 바다 풍경, 명확한 카메라 추적과 전체 조망을 제공하세요.

플레이어 1명이 협력하는 AI 라이벌 2명과 대결합니다. WASD/화살표 키로 이동하고, F로 2.5초짜리 버블을 설치하며, Space로 실제 플랫폼에 점프하고, Shift로 대시하세요. Q/E로 궤도를 회전하고, V로 시점을 변경하며, Escape로 일시정지합니다. 십자형으로 퍼지는 물은 장애물을 따라 반응하고, 첫 번째 부드러운 블록을 부수며 버블을 연쇄적으로 작동시켜야 합니다. 가두기, 탈출, 적 포획, 리스폰, 포획 목표 3/6/9/12회 선택(기본값 6회) 또는 180초 점수제를 구현하고, 결과와 재시작 기능을 제공하세요. 터치 조이스틱과 액션 버튼은 동시에 작동해야 합니다.

생성된 Tripo 픽업 6종을 사용하세요: 풍선, 사거리 물약, 롤러스케이트, 투척 장갑, 킥 부츠와 구조 바늘. 상자에서는 85% 확률로 아이템이 드롭됩니다. 조건부 아이템 가중치는 각각 30/30/30/2.5/3.5/4%입니다. 장갑은 투척 횟수 3회를 추가하며 최대 6회까지 누적됩니다. G 키를 누르면 가까운 버블을 엄폐물 너머 최대 4타일까지 던지고, 버블 소유자와 기존 퓨즈 시간을 유지하며 착지 지점을 예약하고 궤적을 표시하세요. 비행 중 퓨즈가 끝난 버블은 착지한 뒤 폭발합니다. K 키를 누르면 퓨즈를 초기화하지 않고 버블이 막힐 때까지 미끄러뜨립니다. X 키로 구조 바늘을 사용하며, 시작 수량은 1개, 최대 수량은 3개입니다. 인벤토리와 사용 가능한 조작을 명확하게 표시하세요.

중국어, 영어, 한국어 UI를 제공하세요. 중국 본토의 IANA 시간대는 중국어를 선택하고, 한국/북한은 한국어를 선택하며, 홍콩/마카오/대만을 포함한 그 외 모든 지역은 영어를 선택합니다. 수동 선택이 항상 우선합니다. 최대 40 MB, 150k 트라이앵글의 임베디드 텍스처, 표준 비압축 방식, 실제 스키닝이 적용된 GLB만 업로드할 수 있습니다. 본, 관절, 웨이트를 검증하고 정적 모델은 거부하세요. 애니메이션이 포함되어 있으면 이를 사용하고, 클립이 없는 인식 가능한 휴머노이드에는 기본 관절 동작을 제공하세요. 인식할 수 없는 스켈레톤에는 애니메이션이 필요합니다. 리깅과 애니메이션의 차이를 쉽게 설명하고, 90도 방향 조정 기능을 제공하며, 브라우저 내에서 로컬로 처리하세요. 파일 해시를 기준으로 안정적이고 균형 잡힌 능력치를 배정하세요.

생성 CTA에는 정확히 https://studio.tripo3d.ai/?utm_source=satellite_invite&utm_medium=bubble-bay&utm_campaign=create-character를 사용하세요. jared를 크레딧에 표시하고 https://x.com/jaredliu_bravo에 연결하세요.

13종의 ElevenLabs 사운드 효과를 노멀라이즈, 짧은 감쇠, 거리/패닝, 동시 재생 수 제한, 볼륨 및 음소거와 함께 통합하세요: 설치, 폭발, 상자, 픽업, 희귀 아이템, 투척, 착지, 트랩, 구조, 점프, 승리, 패배, 킥. ElevenLabs music_v2_5로 오리지널 인스트루멘털 트랙 2곡을 각각 90초 길이로 제작하고, 항해하는 해적 맵과 햇살 가득한 동네 분위기에 맞추세요. 두 트랙을 절제된 -20 LUFS 목표값으로 노멀라이즈하고 루프 경계를 크로스페이드하세요. 음악 전용 볼륨 조절 기능을 제공하고, 맵에 따라 트랙을 변경하며, 일시정지 시 페이드아웃하고 마스터 음소거를 따르도록 하세요. 오리지널 게임 사운드트랙은 재현하지 마세요.

독립적인 소스, 의존성, 테스트와 에셋 출처 정보는 CMS 플랫폼 앱 외부에 보관하세요. 동일 출처의 정적 리소스와 MIT 고지문을 함께 번들링하세요. Ego Lite에서 실제 데스크톱 및 터치 플레이, 업로드 변형, 언어와 시각 전환을 검증하세요. 기존 CMS Web Page12에 연결된 변경 불가 리뷰 버전을 생성하고, 리뷰가 진행되는 동안 기존 라이브 릴리스를 유지하세요. 정확한 소스와 에셋 권리 확인이 완료된 경우에만 게시한 뒤, 공개 URL, 모델 해시와 동작을 검증하세요. 기록을 보존하고 저장된 CMS 레코드, 프리뷰와 공개 릴리스를 구분하세요.

모델 에셋을 눈에 보이는 진행률과 함께 로드하고, 동시에 다운로드하는 항목은 최대 3개로 제한하며, 유휴 타임아웃은 30초, 시도 횟수는 2회로 설정하세요. 재시도할 때 성공한 다운로드는 보존하세요. 모든 모델이 준비될 때까지 전체 Tripo / Three.js 탭 전환을 숨긴 뒤 준비가 완료되면 표시하세요. 절차적으로 로드하는 플레이스홀더를 로드된 Tripo 모델로 표시하지 마세요.


스폰 지점 바깥에서 시작해 아레나 중앙을 바라보는 근접 시점의 추적 카메라를 사용하세요. Q/E 궤도 회전, 마우스 드래그를 통한 피치 조절과 V 전체 조망을 유지하세요. 상단에는 간결한 스코어보드를, 왼쪽 상단에는 모드 전환을, 하단 근처에는 짧은 알림을 배치하세요. 마우스를 사용하는 데스크톱에서는 조이스틱과 큰 터치 액션 버튼을 숨기되, 작고 드문 아이템 조작은 유지하세요. 좁은 화면에서는 여러 터치 액션을 동시에 사용할 수 있도록 컴팩트한 UI를 지원하세요. 청록색 깊이 변화가 있는 애니메이션 웨이브 셰이더, 곡선형 야자수와 바위가 있는 자연스러운 모래 섬, 은은한 나뭇결과 잔디 텍스처를 추가하세요. 트랩에 걸린 각 캐릭터를 크기에 맞는 반투명 버블로 감싸고, 무지갯빛 프레넬 림, 부드러운 부유 효과, 작은 버블과 바닥 물결을 적용하세요. 포즈가 적용된 스킨 버텍스가 올바르게 맞는지 확인하고, 구조 시 상승값을 초기화하며, 반복적인 전환 후 리소스가 정상적으로 해제되는지 점검하세요. 물막에 비용이 큰 씬 전체 전송 패스를 사용하지 말고, 좁은 화면의 픽셀 비율은 1.5로 제한하세요.
