...
```text
1. 프로젝트 목표
TITANIC — THE LAST LIGHT를 제작합니다. 배의 마지막 일몰부터 충돌, 대피와 침몰, 새벽의 추모 장면까지 이어지는 264초 분량의 인터랙티브 시네마틱 항해입니다. 방문자는 연출된 영화를 감상하고, 움직이는 3D 월드를 탐색하며, 챕터로 이동하고, 스틸 이미지를 저장하거나 완성된 영화를 다운로드할 수 있습니다. 법의학적 정확성이나 공식 제휴를 주장하지 말고 예술적 해석으로 제시합니다.

2. 비주얼 방향
절제된 시네마틱 팔레트를 사용합니다. 짙은 대서양의 푸른색을 배경으로 따뜻한 크림색과 앰버색의 선박 조명을 배치하고, 이후 어두운 별빛의 밤과 차가운 새벽으로 전환합니다. 원근감이 있는 장면을 2.39:1 영화 구도로 렌더링하고, 부드러운 블룸과 은은한 그레인, 비네트를 적용합니다. 캐릭터 초상과 새벽 장면에는 피사계 심도를 사용하되 조난 로켓의 파티클은 선명하게 유지합니다. 바다의 본체 색상은 짙은 파란색으로 설정하고, 월드 공간의 너울, 더 작은 밉맵 리플 노멀, 프레넬 반사를 사용합니다. 따뜻한 일몰 색상은 주로 반사광에 적용합니다. 동일한 바다 변위를 따르는 선체 부착형 웨이크 리본을 사용하고, 끝부분은 부드럽게 처리하며 거품은 끊어진 형태로 만듭니다. 파노라마 텍스처는 수평으로 래핑하되 프랙 불연속이 생기지 않게 하여 하늘의 수직 이음새와 반사 줄무늬를 방지합니다. 주황색 얕은 물 셰이딩, 작고 균일한 리플, 빛나는 원형 폼 데칼은 사용하지 않습니다. 숨겨져 서로 겹치는 지붕 상단 면은 제거해 뎁스 파이팅을 방지하고, 샷 거리에 맞는 카메라 근평면을 사용합니다. 침몰 중에는 고주파 플리커를 추가하지 말고 창문 조명을 단조롭게 어둡게 합니다. 차분한 세리프 제목, 영어/중국어 이중 언어 컨트롤, 화면 하단의 좁은 타임라인을 사용합니다.

3. 월드, 지형 및 카메라 편집
269m 길이의 선박과 선수 방향이 +X인 하나의 연속 좌표계를 사용하고, 빙산은 (275, 0, 57)에 고정합니다. 선박은 전진하다가 96.727초에 빙산과 접촉하고, 관성으로 이동한 뒤 정지하며, 이후 선수부와 선미부로 나뉘어 침몰합니다. 마지막 장면까지 빙산을 유지하고 새벽 구도에서도 보이게 합니다. 0, 63, 110, 163, 211, 241초에 시작하는 6개 챕터를 유지합니다.

의도적으로 구성한 23개의 샷을 제작합니다. 선수 포옹 장면은 29~61초에 걸쳐 구성합니다. 도입용 접근 샷, 두 인물의 클로즈업 더블 포트레이트, 뒤에서 바다를 바라보는 뷰, 사선 포트레이트를 포함합니다. 선수 끝부분에 Rose를 앞에, Jack을 바로 뒤에 배치하고 두 사람 모두 선수 너머를 바라보게 합니다. 시퀀스 전체에서 일몰 조명을 유지합니다. 127~158초에는 구명정의 실제 월드 트랜스폼에 연결된 세 개의 샷을 사용합니다. 보트 데크에서 출발하는 장면, 승객과 서스펜션 로프를 더 가까이 보여 주는 장면, 수면으로 접근하는 장면입니다. 이어서 와이드 대피, 선체 기울어짐, 파손, 침몰 샷을 배치합니다. 새벽에는 멀리 빙산이 보이는 생존 구명정들을 보여 준 다음 절제된 추모 제목을 표시합니다.

4. 에셋 목록
- titanic-vessel: Blender에서 269m 길이의 주요 구조물을 제작합니다. 연속된 웰 데크, 폐쇄형 포캐슬, 층층이 구성된 프로머네이드, 속이 빈 경사형 버프 퍼널 4개, 검은색 상부 선체와 빨간색 하부 선체를 포함합니다. 864개 현창에는 실제 원형 림을, 360개 창문에는 프레임을 사용합니다. 이미션은 실제 유리 재질에만 제한합니다. 머티리얼을 배치 처리하고 x=-32에서 분할해 침몰에 사용합니다. 마스트, 리깅, 데빗, 데빗 로프, 청동 프로펠러와 방향타를 추가합니다. 티크 문과 황동 디테일이 있는 P2 컴패니언웨이를 생성하고, 정규화한 뒤 제작된 데크에서 두 번 재사용합니다. GLB를 조립할 때 노드 트랜스폼을 보존합니다.
- atlantic-iceberg: 불규칙하고 침식된 청백색 빙산 하나를 제작합니다. 층층이 쌓인 서리, 거칠기 변화, 절제된 노멀 맵, 설득력 있는 수선을 적용합니다. 지리적으로 고정된 오브젝트로 유지합니다.
- lifeboat: 흰색 목재 선체, 어두운 건현 가장자리, 벤치와 노를 갖춘 White Star 조정식 구명정 하나를 제작하고, 독립적으로 움직이는 16척의 보트에 인스턴싱합니다.
- bow-embrace: 요청된 1997년 영화의 의상과 포즈에서 영감을 받은 격리형 두 인물 에셋입니다. Rose는 적갈색 머리, 네이비/아이보리 의상과 패턴 숄을 착용하고 팔을 뻗은 자세로 배치하며, Jack은 바로 뒤에서 어두운 코트와 아이보리 셔츠를 입습니다. 3D 변환 전에 깔끔한 레퍼런스 이미지를 생성합니다. 깨끗한 머리-목-어깨 레퍼런스를 바탕으로 각 주인공의 클로즈업 머리, 전체 목, 상흉부를 하나의 연속된 H v3.1 서피스로 제작합니다. P2 포옹 포즈와 손, 의상을 유지합니다. 쿼드 리토폴로지, Blender 로컬 서피스 정리, 4K 컬러/노멀 베이킹을 사용합니다. 턱 바로 아래에 직접 결합하지 말고 의상 개구부 아래에 인터페이스를 맞춥니다. 생성된 연속 목 구조를 보존합니다. 원래 의상 아래에서 측정한 몸통 윤곽에 하흉부를 맞추고, 연속된 P2 상부 등을 복원하며, 불필요한 머리카락 잔여물을 제거합니다. 떠 있는 칼라 패널과 늘어진 UV 목 돌출부는 피합니다. 낮은 측면 뷰와 후면 뷰 모두에서 빛이 새는 틈이나 노출된 절단면이 없는지 확인합니다. 제작된 카메라 뷰에서 눈꺼풀, 홍채, 입술, 손, 텍스처 이음새를 검사합니다. 천의 스쳐 지나가는 각도에서 생기는 하이라이트는 피부 셰이딩과 별도로 줄입니다. 자연스러운 기본 표정과 절제된 몸 및 천의 움직임을 사용하며, 실제로 구현하지 않은 얼굴 애니메이션 리그를 암시하지 않습니다.
- seated-woman 및 seated-man: 1912년 복장과 밝은 코르크 구명조끼를 착용한 성인 승객 모델을 각각 제작합니다. 무릎을 굽혀 앉고 손은 무릎 위에 둡니다. 보트 간에 지오메트리와 머티리얼을 공유하되 배치와 방향은 약간씩 다르게 합니다. 승선을 위한 보트별 인스턴스 수를 되돌릴 수 있게 구성합니다.

주요 선박에는 Blender를, 데크 컴패니언웨이와 빙산, 보트, 의상, 승객에는 Tripo P2.0을, 두 개의 연속된 주인공 포트레이트에는 H v3.1을 사용합니다. 바다, 하늘 블렌드, 별, 조명, 연기, 조난 로켓, 거품, 물보라와 잔해는 씬 이펙트로 유지합니다. 압축 텍스처를 적용한 더 가벼운 웹용 모델 변형을 제공하고, 오프라인 영화 출력에는 원본 해상도 에셋을 유지합니다.

5. 재생 및 피드백
필수 선박, 보트, 하늘, 바다 노멀 에셋의 실제 로딩 진행률을 표시합니다. 첫 번째 씬이 준비되면 시작 버튼을 활성화하고, 나머지 모델과 음악은 지연 로드합니다. 필수 캐릭터나 빙산 모델의 로딩이 늦어지면 해당 샷을 조용히 건너뛰지 말고 씬 경계에서 대기한 뒤 준비되는 즉시 재개합니다. 오디오는 사용자가 상호작용한 후에 시작합니다.

타임라인은 0초로 초기화되지 않고 앞뒤 탐색과 빠른 드래그를 지원해야 합니다. 탐색 중에도 재생/일시정지 및 음소거 상태를 유지하고, 이전 오디오 클록이 요청된 위치를 덮어쓰지 않게 합니다. MP3와 MP4에는 바이트 범위 요청을 제공합니다. 챕터 이동에는 29초의 선수 포옹과 127초의 구명정 하강으로 바로 이동하는 항목을 포함합니다. 이 항목들은 디렉터 카메라를 복원하면서 재생 상태는 유지합니다.

탐색 중에도 월드, 선박과 사운드트랙은 계속 재생되며 오빗, 드래그, 줌을 사용할 수 있습니다. 시점 방향을 갑자기 튕기지 말고 선박의 이동을 따라갑니다. 일시정지는 독립적으로 유지하며 영화로 돌아오면 현재 시간을 보존합니다. Space는 재생/일시정지, 화살표 키는 10초 이동, M은 사운드 전환, E는 탐색 모드 전환, F는 전체 화면을 실행합니다. 터치 오빗/핀치와 타임라인 탭도 지원합니다.

보트는 빈 상태로 시작합니다. 승객은 112초 이후 여러 그룹으로 나뉘어 순차적으로 승선하고, 각 보트가 하강하기 전에 승선을 완료합니다. 로프는 움직이는 데빗과 실제 보트 부착 지점 사이를 연결하고, 분리 후에는 사라집니다. 뒤로 탐색하면 이전 승객 탑승 상태와 로프 상태를 복원합니다. 충돌 시 선체/카메라 진동, 얼음 조각, 긁히는 물보라, 강철과 얼음이 부딪히는 사운드 트랜지언트를 서로 맞물리게 연출합니다. 조난 로켓은 흰색으로 타오르는 별, 짧은 개별 트레일, 중력, 드래그, 서서히 사라지는 연기를 사용합니다. 침몰로 인한 교란은 불규칙하고 파도를 따라 움직이며 점진적으로 감쇠하는 패치로 표현합니다. 물보라는 멀리 떨어진 포인트 분수에서 생성하지 말고 선미의 실제 수선을 따라 분포시킵니다.

6. 기술 구현 및 납품물
Vite, JavaScript 모듈, Three.js를 사용하고 시간 기반 애니메이션을 결정론적으로 구현합니다. 카메라/타임라인, 선박 에셋, 캐릭터, 환경, 이펙트, 씬 준비 상태를 분리합니다. 웹 재생, 탐색, 오프라인 캡처에 동일한 시간 모델을 사용합니다. 웹 렌더링은 픽셀, 반사, 그림자 예산을 명시적으로 제한하고, 더 무거운 앰비언트 오클루전은 오프라인 프로파일로 미룹니다. 장시간 블로킹되는 시작 전 프리웜 없이 에셋을 컴파일하고 디코딩합니다. 스크립트, 모델, 이미지, 폰트, 오디오를 동일한 오리진에서 호스팅하고 정적 빌드에 비밀 정보를 포함하지 않습니다.

오리지널 스코어와 유료 ElevenLabs Foley를 사용합니다. 완성된 자연스러운 휘파람 불꽃놀이 테이크를 실제 공중 폭발 지점에서 나누어 상승 비행 구간과 날카로운 폭발음 및 바스락거리는 잔향으로 구성하고, 강철/얼음 접촉 및 긁힘, 구명정 로프와 수면 접촉, 선체의 변형/파손, 선미의 물 변위를 포함합니다. 소스 WAV 파일을 출력하고 프롬프트/히스토리 ID를 보존한 뒤 시간에 맞는 큐로 편집합니다. 발사는 119, 151, 183초에 맞추고 공중 폭발은 각각 3.15초 후에 발생하게 합니다. 원래 폭발음의 어택을 유지하고 오케스트라 스코어는 잠시 덕킹합니다. 믹스 아래에는 잔잔한 바다, 바람, 엔진 앰비언스를 유지합니다. 공용 웹사이트와 다운로드 가능한 영화 모두에 대한 승인이 없다면 레퍼런스 영화의 사운드트랙을 포함하지 않습니다. 폴백을 서비스 생성 에셋으로 설명하지 말고 실제 에셋 및 사운드 출처를 문서화합니다.

3840×2160, 24fps, 템포럴 샘플 3개로 총 6,336프레임을 결정론적으로 출력합니다. 영어 제목을 번인하고 2.39:1 레터박스 이미지를 사용합니다. 4K H.264/AAC 마스터와 100 MiB 미만의 1080p 웹 버전을 인코딩하며, 두 버전 모두 길이는 264초, 오디오는 48kHz 스테레오로 구성합니다. 선택 가능한 중국어/영어 자막, 오디오 마스터, 편집 가능한 소스를 보존합니다. 플랫폼 애플리케이션이나 페이지별 Worker를 추가하지 말고 기존 CMS Web Pages 호스팅을 통해 정적 빌드를 게시합니다.

7. 검수 기준
오프닝, 두 캐릭터 포트레이트, 충돌, 조난 로켓, 세 개의 하강 샷, 파손, 선미 소실, 새벽 장면을 검사합니다. 244초에 빙산이 사라지지 않는지, 바다가 짙은 파란색으로 보이는지, 침몰 장면에 규칙적인 흰색 고리가 없는지, 제작된 샷에 캐릭터가 계속 등장하는지, 하강 내내 보트 로프와 승객의 정렬이 유지되는지 확인합니다. 모델 지연 로딩, 앞뒤 탐색, 빠른 스크러빙, 일시정지/음소거, 동적 탐색, 클로즈업 직접 진입, 터치 에뮬레이션을 테스트합니다. 출력된 모든 프레임을 검증하고 두 영화를 완전히 디코딩하며, 실제 브라우저 다운로드 파일과 납품 파일을 비교하고, 공개 빌드와 CMS 연결을 확인합니다. 브라우저 모바일 에뮬레이션과 실제 휴대폰 테스트를 구분합니다.
