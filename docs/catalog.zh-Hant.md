...
<a href="https://www.tripo3d.ai/zh-Hant/3d-prompts/bubble-bay"><img src="../assets/previews/3f9d3b8fcab4a54ff4bf19c6f25eb3f3ce6ce697cb929ab0abf86e89d6b8f708.jpg" width="840" loading="lazy" alt="泡泡小鎮：3D 水球大戰"></a>

<a href="https://www.tripo3d.ai/zh-Hant/3d-prompts/bubble-bay"><img src="../assets/previews/a0927800e707bb993467cd9c9d32d7955cafcad0fe1ab3986c33d6bec6ce85f8.jpg" width="840" loading="lazy" alt="泡泡小鎮：3D 水球大戰"></a>

<a href="https://www.tripo3d.ai/zh-Hant/3d-prompts/bubble-bay"><img src="../assets/previews/44f0d05c544d89c5044aece0e9da8cc3730bc51d81f395ff0e554bbceb4f2ab2.jpg" width="840" loading="lazy" alt="泡泡小鎮：3D 水球大戰"></a>

在 Pirate14 與 Village10 上打造經典 3D 水球大戰，加入三名具備動畫的 P2 角色、近距離視角、虹彩陷阱泡泡、可設定的捕獲目標、AI 對手、自訂角色匯入，以及 ElevenLabs 原創音樂。預設捕獲數為 6；載入完成後才顯示比較分頁。

**提示詞**

```text
打造 Bubble Bay，一座可遊玩的 Three.js 水球競技場，讓人立刻聯想到經典 BnB。預設使用高細節 Tripo 模型，並提供一個清楚易用的 Three.js 幾何比較切換功能，且不影響對戰內容。使用提供的 Boz/Bazzi、Ike/Uni 與 Plunk/Keffy 原始角色參考圖，保留臉部特徵、頭飾、色彩、服裝與身體比例，不要以僅有些許相似的文字生成設計取代。

使用 Tripo CLI 搭配明確的 tripo-p2 生成角色與道具幾何。準備中性、正面朝向的參考姿勢，生成模型、綁定具有有效蒙皮權重的真實雙足骨架，並匯出 GLB。保留工作 ID、點數、原始蒙皮檔案與最終雜湊值。待機、跑步與跳躍必須驅動實際關節產生動畫。修正過大的頭部、耳朵與手套的蒙皮問題，並檢查多個視角與動作。必須準確描述本地製作的動畫片段，不得稱為 Tripo 預設內容。原始角色的公開使用權需要另行確認；AI 生成不等於取得使用許可。

本地化名稱：宝宝 / 배찌 / Boz (Bazzi)、小乖 / 우니 / Ike (Uni)、胖墩 / 케피 / Plunk (Keffy)。初始容量／範圍／速度等級：1/1/5、1/2/5、1/2/4；上限：6/7/9、6/7/8、9/8/8。將速度換算為每秒 0.25 + 等級*0.8 個世界單位；每格為 2 個單位。選擇角色後，將另外兩名角色指定為資料各異但設定相符的對手。

預設提供 15x13 的 Pirate/Patrit14 與 Village10。保留容易辨識的金色甲板、黃色貨物、木箱、四門大砲與中央桅杆；村莊則包含四個不同色彩的住宅區、中央道路、樹籬與玩具積木。以發行商地圖作為參考，自行建構執行期間的場景美術，並記錄為維持 3D 連續移動與 AI 逃脫所需的小型路徑開口。提供明亮材質、陰影、海洋景觀、清楚的鏡頭跟隨與總覽視圖。

一名玩家對上兩名合作的 AI 對手。使用 WASD／方向鍵移動，F 放置持續 2.5 秒的泡泡，Space 跳上實際平台，Shift 衝刺，Q/E 環繞鏡頭，V 切換視角，Escape 暫停。十字形水流會遵守障礙物、破壞第一個軟方塊並串連泡泡。實作困住、逃脫、敵方捕獲、重生，以及可選的 3、6、9 或 12 次捕獲目標（預設為 6 次），或 180 秒計分模式，並提供結果與重試功能。觸控搖桿與動作按鈕必須能同時使用。

使用六種生成的 Tripo 道具：氣球、範圍藥水、溜冰鞋、投擲手套、踢擊靴與救援針。木箱有 85% 的機率掉落道具。條件式道具權重：30/30/30/2.5/3.5/4%。手套增加三次投擲次數，上限為六次。G 可將附近的泡泡拋過掩體，最遠四格，保留泡泡的持有者與原始引信時間，預留落點並顯示拋物線。飛行中的泡泡若引信倒數結束，會在落地時爆炸。K 可讓泡泡滑行，直到受阻為止，但不重設引信時間。X 使用救援針，初始數量為一，上限為三。清楚顯示道具欄與可用控制方式。

提供中文、英文與韓文介面：中國大陸的 IANA 時區選擇中文，韓國／北韓選擇韓文，其餘地區（包括香港／澳門／臺灣）選擇英文。手動選擇永遠優先。僅允許上傳內嵌貼圖、標準未壓縮且確實具備蒙皮的 GLB，大小上限為 40 MB、15 萬個三角形。驗證骨骼、關節與權重，拒絕靜態模型；若模型包含動畫，使用其內嵌動畫；沒有動畫片段時，為可辨識的人形模型提供基本關節動作。無法辨識的骨架必須具備動畫。清楚說明骨架綁定與動畫的差異，提供 90 度方向調整，在瀏覽器內本機處理，並依檔案雜湊值指派穩定且平衡的屬性。

建立 CTA 時必須使用 https://studio.tripo3d.ai/?utm_source=satellite_invite&utm_medium=bubble-bay&utm_campaign=create-character。Credit jared 連結至 https://x.com/jaredliu_bravo。

整合 13 種 ElevenLabs 音效，支援正規化、短衰減、距離／聲像、複音數量限制、音量與靜音：放置、爆裂、木箱、拾取、稀有道具、投擲、落地、陷阱、救援、跳躍、勝利、失敗、踢擊。使用 ElevenLabs music_v2_5 創作兩首原創器樂曲，每首 90 秒，分別符合航海海盜地圖與陽光住宅區的氛圍。將音樂正規化至克制的 -20 LUFS 目標值，並對循環邊界套用交叉淡化。音樂使用獨立音量控制，依地圖切換曲目，暫停時淡出，並遵守主音量靜音設定。不得重現原遊戲配樂。

將獨立的原始碼、相依套件、測試與資產來源資訊保留在 CMS 平台應用程式之外。將同源靜態資源與 MIT 授權聲明一併打包。在 Ego Lite 中驗證實際的桌面與觸控遊玩、上傳變體、語言與視覺切換。建立一個不可變更的審查版本，附加至現有的 CMS Web Page12；審查期間保留現有的正式版本。只有在原始碼與資產權利獲得如實確認後才發布，接著驗證公開 URL、模型雜湊值與行為。保留歷史紀錄，並區分已儲存的 CMS 記錄、預覽與公開版本。

載入模型資產時顯示進度，同時最多下載三個檔案，閒置逾時為 30 秒，最多嘗試兩次。重試時保留已成功下載的檔案。所有模型準備完成前，隱藏整個 Tripo／Three.js 分頁切換；完成後才顯示。不得將程序式載入中的佔位模型標示為已載入的 Tripo 模型。


使用近距離視角的跟隨鏡頭，從出生點外開始，朝向競技場中央。保留 Q/E 環繞、滑鼠拖曳俯仰與 V 總覽視圖。頂端放置精簡的計分板，左上角放置模式切換，底部附近顯示簡短通知。在滑鼠桌面裝置上隱藏搖桿與大型觸控動作按鈕，但保留小型稀有道具控制；在窄螢幕上支援精簡的同步觸控操作。加入具動畫效果的波浪著色器，呈現青綠色深度變化；製作帶有彎曲棕櫚樹與岩石的自然沙質小島、細緻木紋與草地貼圖。每名被困角色都要包覆在符合角色尺寸的半透明泡泡中，泡泡具備虹彩 Fresnel 邊緣、輕柔漂浮效果、小泡泡與地面波紋。確認姿勢下的蒙皮頂點貼合模型，救援時重設抬升效果，並檢查反覆切換後的資源釋放。避免對整個場景執行成本高昂的水膜透射處理；窄螢幕的像素比例上限設為 1.5。
