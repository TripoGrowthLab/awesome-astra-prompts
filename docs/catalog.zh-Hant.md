...
<a href="https://www.tripo3d.ai/zh-Hant/3d-prompts/bubble-bay"><img src="../assets/previews/5158051d1bd3d80bc65cbfdd00a31c6e9c048c21f0e18ed7b1826972c94b6b5d.webp" width="840" loading="lazy" alt="泡泡小鎮：3D 水球大戰"></a>

<a href="https://www.tripo3d.ai/zh-Hant/3d-prompts/bubble-bay"><img src="../assets/previews/ceda95626d7f2a404ab04a28ea483f2da5eb9cc367184a09da386305cc85539a.png" width="840" loading="lazy" alt="泡泡小鎮：3D 水球大戰"></a>

在陽光明媚的 3D 庭院中打造水球大戰，加入跳躍、AI 對手和自己的動畫角色。在 Tripo 模型與 Three.js 幾何版本之間切換。

**提示詞**

```text
1. 專案目標
打造 泡泡小鎮，一款可遊玩的 3D 水球大戰遊戲，場景位於陽光明媚的玩具城庭院。玩家操控一名藍隊角色，對抗兩名紅隊 AI 對手。穿梭於各條路線、破壞木箱、收集強化道具、跳上平台，並用泡泡困住對手。在三分鐘的對戰中捕捉對方隊伍三次即可獲勝。交付完整的 Three.js 遊戲，提供清楚可見的視覺模式比較，以及可將個人骨架綁定角色帶入對戰的角色工坊。

2. 視覺方向
以附上的庭院螢幕截圖與遊戲錄影作為目標版本。採用貼近角色的第三人稱視角，相機視野約 51 度，從角色上方與後方平順跟隨。讓可遊玩空間填滿畫面。使用奶油色石板鋪面、蜂蜜黃色木箱、紅磚花台、藍色與陶土色小屋屋頂、圓潤的綠色樹木，以及成簇的小花。讓三名角色具備清楚易辨的剪影，表面採用柔和陰影與貼圖。加入溫暖的定向陽光、柔和的接觸陰影、輕微霧氣、克制的類泛光水面高光，以及透明藍色泡泡。即使場景細節豐富，也要確保分數、計時器、操作方式與對手清楚易讀。
主畫面在左側覆蓋精簡的角色選擇器、三項屬性指示、難度選項與醒目的開始按鈕。將 Tripo 與 Three.js 視覺模式置於頂部中央的醒目位置；切換標籤、選取樣式與顯示模型必須同步變更。預設使用 Tripo 模式。遊戲進行中，將介面精簡為隊伍分數、計時器、玩家標記、即時屬性與操作提示。使用置中的結果面板，顯示分數、最佳成績、重玩與角色工坊操作。

3. 世界與配置
打造約 22 × 24 世界單位的矩形庭院，格子間距為 2 個單位。保留清楚的中央南北向通道，並在南側邊緣附近設置安全起始區域。在連通路線周圍交錯配置實心花台與可破壞木箱。沿周邊散布小屋、樹木與花朵，但不要遮擋遊玩區域。使用獨立於詳細裝飾網格的簡單碰撞體。花台支撐高度為 1.02 個單位，木箱為 1.72 個單位。角色必須確實落在它們的頂部，並在走出邊緣時落下。木箱遭破壞後消失，開啟新的路線。在南側附近生成玩家，並在北半部生成兩名對手。提供可抵達的初始強化道具，且每場對戰變更木箱配置。

4. 資產清單
使用可獨立替換的 GLB 插槽，統一其邊界、朝向與接地樞紐。優先處理角色，其次是重複使用的木箱與小屋，最後是植被。
- hero：Blue，預設的藍隊卡通冒險者，頭部圓潤、身形精簡。使用一個骨架綁定角色資產，預設作為藍隊玩家；支援待機、跑步與跳躍。
- mint：Mint，小型青綠色奇幻生物，頭部帶有粉紅色裝飾。使用一個骨架綁定角色資產，可由玩家選擇，也可供對手使用；支援待機與跑步。
- mango：Mango，橘色動物風格吉祥物，臉部與腹部為奶油色。使用一個骨架綁定角色資產，可由玩家選擇，也可供對手使用；支援待機與跑步。
- cottage：奶油色牆面的小型小屋，搭配厚實的瓦片斜屋頂。在邊界周圍重複放置；重複使用幾何結構，並套用藍色或陶土色屋頂變體。
- crate：蜂蜜色木箱，具有木板細節與堅固的剪影。在可破壞格子中重複放置；木箱破損或從不同角度觀看時，內部仍要維持封閉且可見的結構。
- tree：短小的風格化樹木，具有粗樹幹與圓潤、分層的樹葉。在庭院邊界後方重複放置。
- bush：低矮的綠色植被，帶有小型彩色花朵。在花台內與房屋周圍重複放置。
地面鋪面、花台磚牆、天空、燈光、碰撞代理、泡泡、十字形水花爆發效果、粒子、拾取道具與所有介面元素都必須以程序化方式製作。為 Three.js 模式提供場景與角色相符的幾何版本。切換視覺模式時，必須保留對戰狀態、位置、計時器與分數。

5. 遊戲玩法與回饋
使用基於 delta time 的連續移動、正規化的斜向速度、碰撞偵測與平順的跟隨相機。鍵盤操作：使用 WASD 或方向鍵移動；按 F 放置泡泡；按 Space 跳躍；按 Q/E 旋轉相機；拖曳場景調整相機方向與俯仰角；按 Esc 暫停；按 Shift 觸發持續 0.3 秒、冷卻時間 4 秒的衝刺。在觸控螢幕上提供左側搖桿，以及右側分開的泡泡與跳躍按鈕，且按鈕可同時使用。
泡泡會在地面格子上佔據 2.5 秒，接著沿四個格子方向發出十字形水花爆發。實心花台與邊界會阻擋水花。每個方向上第一個被擊中的木箱會破壞並阻止該方向的水花延伸。附近的泡泡會連鎖引爆。允許放置者離開新放置的泡泡，但禁止其穿回泡泡所在位置。遵守同時存在的泡泡數量上限，且角色離地時禁止放置泡泡。
水會將角色困住 3.5 秒。連續按四次 Space 可釋放角色；按 X 會消耗每場對戰一枚救援針，立即脫困。敵方角色碰觸被困角色時會立即捕捉；否則陷阱時間結束後，敵方隊伍獲得一次捕捉，受害者重新生成。紅隊隊友不會互相捕捉或造成傷害。若角色困住自己，陷阱時間結束時敵方隊伍仍會獲得一次捕捉。跳到高處或平台頂部可以避開低處的水花爆發。
初始配置：Blue 擁有 2 個泡泡、範圍 2、移動速度 4.25；Mint 擁有 1 個泡泡、範圍 2、速度 5.15；Mango 擁有 1 個泡泡、範圍 3、速度 4.55。泡泡、範圍與速度拾取道具會提升相應屬性。提供清楚易讀的拾取回饋，以及反應靈敏的水花、移動、跳躍、受困與捕捉效果。加入簡短的合成音效，並提供靜音控制。
最先達到 3 次捕捉的隊伍獲勝。180 秒時比較隊伍分數；分數相同則判定平手。加入能夠移動、避開危險、放置泡泡與追捕對手的簡單與普通難度對手。暫停時凍結模擬；重玩時重設對戰。將最佳成績儲存在本機。

6. 技術實作與角色工坊
使用 Three.js 0.180、JavaScript ES modules 與 Vite。分離模擬、場景渲染、模型載入、輸入／介面與翻譯功能。將執行時依賴、GLB、貼圖與通知內容打包為同源靜態檔案；遊玩不需要帳號或伺服器。使用 GLTFLoader、SkeletonUtils 複製功能與 AnimationMixer 處理骨架綁定模型，並為重複使用的環境資產共用或重複利用幾何與材質。交付的專案使用 Tripo P2 生成的角色與環境資產；保留其來源資訊，並確保骨架與內嵌動畫維持正常運作。
提供中文、英文與韓文介面。有效儲存的手動語言選擇永遠優先。否則使用裝置的 IANA 時區：中國大陸時區（例如 Asia/Shanghai 與 Asia/Urumqi）選擇中文；Asia/Seoul 與 Asia/Pyongyang 選擇韓文；香港、澳門、臺灣及所有其他或無法取得的時區選擇英文。一致處理舊版別名。不得根據 UTC 偏移或瀏覽器語言推斷中國大陸。不得將自動選擇結果儲存為手動偏好。
角色工坊接受大小上限 40 MB、內嵌貼圖的 GLB，並在加入角色前驗證實際的蒙皮、骨架、關節與權重。拒絕未綁定骨架的靜態模型，並清楚說明原因。在瀏覽器中於本機解析模型。根據檔案內容雜湊產生穩定的初始配置，讓相同檔案每次都取得相同且平衡的屬性。提供 90 度朝向調整功能。
說明必須具備骨架綁定與蒙皮，但對於支援的標準人形骨架，分開的動畫片段為選用項目：優先使用內嵌的待機／跑步／行走／跳躍動畫片段，否則透過可辨識的肢體骨骼驅動基本待機與行走。無法辨識或非人形骨架必須包含可用的動畫片段。不要承諾能為任意骨架自動製作動畫。說明不支援的壓縮 GLB 格式，以及如何匯出標準未壓縮檔案。重新整理後，已上傳的角色需要再次選取。
提供清楚的建立角色操作，連結至 https://studio.tripo3d.ai/?utm_source=satellite_invite&utm_medium=bubble-bay&utm_campaign=create-character 。說明建立、骨架綁定、匯出與返回遊戲的步驟。將這些說明保留在角色工坊中，不要因此阻擋玩家進行第一場對戰。

7. 驗收與交付
交付可編輯原始碼、依賴鎖定檔、README、本機執行說明、正式環境靜態建置版本，以及可存取的遊玩網址。驗證主畫面、進行中的戰鬥、受困狀態、水花爆發、平台著地、結果畫面、暫停與重玩功能。在不重設遊戲進度的情況下，確認 Tripo／Three.js 之間存在明顯的視覺差異。實際測試鍵盤輸入與觸控模擬，包括同時移動與放置泡泡；誠實標示模擬測試，不得宣稱涵蓋實體裝置。測試角色匯入，包括未綁定骨架的拒絕、帶動畫的骨架，以及沒有動畫片段但受支援的骨架。檢查中國大陸／香港／澳門／臺灣／韓國／其他時區，以及手動選擇的持久化。驗證實際託管頁面與同源資產、角色工坊連結及嵌入使用情境。保留與交付版本庭院相符的實際遊戲錄影與代表性螢幕截圖；不得以早期的遠距離島嶼配置替代。保留作者／來源署名與第三方授權聲明，但不得重新散布參考網站的資產。

```

[查看詳情 ↗](https://www.tripo3d.ai/zh-Hant/3d-prompts/bubble-bay) · [線上展示](https://bubble-bay.tripo.page/) · [返回案例導覽](#all-prompts)
...
<tr><td align="center">
<br>
<p><strong><a href="https://www.tripo3d.ai/zh-Hant/3d-prompts/models/gpt-6-astra?utm_source=github&amp;utm_medium=referral&amp;utm_campaign=awesome_astra_prompts&amp;utm_content=catalog_bottom">前往官網查看全部 264 個案例 →</a></strong></p>
