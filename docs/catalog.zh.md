...
<a href="https://www.tripo3d.ai/zh/3d-prompts/bubble-bay"><img src="../assets/previews/3f9d3b8fcab4a54ff4bf19c6f25eb3f3ce6ce697cb929ab0abf86e89d6b8f708.jpg" width="840" loading="lazy" alt="泡泡小镇：3D 水球大战"></a>

<a href="https://www.tripo3d.ai/zh/3d-prompts/bubble-bay"><img src="../assets/previews/a0927800e707bb993467cd9c9d32d7955cafcad0fe1ab3986c33d6bec6ce85f8.jpg" width="840" loading="lazy" alt="泡泡小镇：3D 水球大战"></a>

<a href="https://www.tripo3d.ai/zh/3d-prompts/bubble-bay"><img src="../assets/previews/44f0d05c544d89c5044aece0e9da8cc3730bc51d81f395ff0e554bbceb4f2ab2.jpg" width="840" loading="lazy" alt="泡泡小镇：3D 水球大战"></a>

在 Pirate14 和 Village10 上打造经典的 3D 水球大战，加入 3 个带动画的 P2 角色、近距离视角、虹彩陷阱泡泡、可配置的捕获目标、AI 对手、自定义角色导入，以及原创 ElevenLabs 音乐。默认完成 6 次捕获；加载完成后再显示对比选项卡。

**提示词**

```text
打造 Bubble Bay：一个可玩的 Three.js 水球竞技场，让人立刻联想到经典 BnB。默认使用高细节 Tripo 模型，并提供醒目的 Three.js 几何体对比开关，切换时保持当前对局。使用提供的 Boz/Bazzi、Ike/Uni 和 Plunk/Keffy 原始角色参考图，保留面部特征、头饰、颜色、服装和身体比例，不要用仅有宽泛灵感的文本生成设计替代它们。

使用 Tripo CLI 和明确的 tripo-p2 生成角色与道具几何体。准备中性、正面朝向的参考姿势，生成模型，为其绑定真实的双足骨骼并设置有效的蒙皮权重，导出 GLB。保留任务 ID、积分、原始蒙皮文件和最终哈希值。待机、跑步和跳跃必须驱动真实关节产生动画。修正过大的头部、耳朵和手套的蒙皮问题，并检查多个视角和运动状态。本地制作的动画片段必须准确描述，不能标注为 Tripo 预设。原始角色的公开使用权需要单独确认；AI 生成不等于获得使用许可。

本地化名称：宝宝 / 배찌 / Boz (Bazzi)、小乖 / 우니 / Ike (Uni)、胖墩 / 케피 / Plunk (Keffy)。初始容量/范围/速度等级：1/1/5、1/2/5、1/2/4；上限：6/7/9、6/7/8、9/8/8。将速度换算为每秒 0.25 + 等级*0.8 个世界单位；每个地块为 2 个单位。选择一个角色后，将另外两个角色分配为属性配置匹配且彼此不同的对手。

默认提供 15x13 的 Pirate/Patrit14 和 Village10。保留易于辨认的金色甲板、黄色货物、木箱、4 门大炮和中央桅杆；村庄包含 4 个不同颜色的住宅区、中央道路、树篱和积木。以发行方地图为参考，自行构建运行时美术资源，并记录为实现连续 3D 移动和 AI 逃脱所需的小型路线开口。提供明亮的材质、阴影、海洋场景、清晰的跟随镜头和总览视图。

1 名玩家对战 2 名协作型 AI 对手。使用 WASD/方向键移动，F 放置一个持续 2.5 秒的泡泡，空格键跳上真实平台，Shift 冲刺，Q/E 环绕，V 切换视角，Escape 暂停。十字形水流应遵循障碍物，破坏第一个软方块并串联泡泡。实现困住、逃脱、敌方捕获、重生，可选择捕获 3、6、9 或 12 次（默认 6 次），或进行 180 秒计分；提供结果和重试功能。触摸摇杆和动作按钮必须能够同时使用。

使用 6 种生成的 Tripo 道具：气球、范围药水、旱冰鞋、投掷手套、踢击靴和救援针。木箱有 85% 的概率掉落道具。条件道具权重：30/30/30/2.5/3.5/4%。手套增加 3 次投掷次数，上限为 6 次。G 键可将附近的泡泡向上投掷，越过掩体最多 4 个地块；保留泡泡的所有者和原始引线时长，预留落点并显示抛物线。飞行中的泡泡到时后会落地并爆炸。K 键可让泡泡滑行，直到被阻挡，但不会重置引线时长。X 键使用救援针，初始为 1 个，上限为 3 个。清晰显示物品栏和可用控制。

提供中文、英文和韩文界面：IANA 时区属于中国大陆时选择中文，属于韩国/朝鲜时选择韩文，其他地区（包括中国香港、中国澳门和中国台湾）选择英文。手动选择始终优先。仅允许上传内嵌纹理、标准未压缩、确实带蒙皮的 GLB，大小最多 40 MB、最多 150k 个三角形。验证骨骼、关节和权重，拒绝静态模型；有内嵌动画时使用这些动画；没有动画片段时，为已识别的人形骨架提供基础关节运动。无法识别的骨架必须提供动画。清楚说明骨骼绑定与动画的区别，提供 90 度朝向调整，在浏览器本地处理，并根据文件哈希分配稳定且平衡的属性。

创建 CTA 必须使用 https://studio.tripo3d.ai/?utm_source=satellite_invite&utm_medium=bubble-bay&utm_campaign=create-character。将 jared 署名链接至 https://x.com/jaredliu_bravo。

集成 13 种 ElevenLabs 音效，并支持归一化、短衰减、距离/声像、复音数限制、音量和静音：放置、爆裂、木箱、拾取、稀有道具、投掷、落地、困住、救援、跳跃、胜利、失败、踢击。使用 ElevenLabs music_v2_5 创作 2 首原创器乐曲，每首 90 秒，分别匹配航海主题的海盗地图和阳光明媚的社区。将音乐归一化至克制的 -20 LUFS 目标值，并对循环边界进行交叉淡化。为音乐提供独立音量控制，根据地图切换曲目，暂停时淡出，并遵循主静音设置。不要复现原游戏配乐。

将独立源码、依赖、测试和资源出处保存在 CMS 平台应用之外。打包同源静态资源和 MIT 许可声明。在 Ego Lite 中验证实际的桌面端和触摸端玩法、上传变体、语言和视觉切换。创建一个附加到现有 CMS Web Page12 的不可变审查版本；审查期间保留现有线上版本。只有在源码和资源权利获得如实确认后才发布，然后验证公开 URL、模型哈希值和运行行为。保留历史记录，并区分已保存的 CMS 记录、预览和公开发布版本。

加载模型资源时显示进度，最多同时下载 3 个资源，空闲超时设为 30 秒，并尝试 2 次。重试时保留已成功的下载。必须等所有模型准备就绪后，才显示完整的 Tripo / Three.js 选项卡切换；随后再显示该切换。不要将程序化加载占位模型标记为已加载的 Tripo 模型。


使用近距离视角的跟随镜头，镜头初始位于出生点外并朝向竞技场中心。保留 Q/E 环绕、鼠标拖拽俯仰和 V 总览视图。顶部放置紧凑的计分板，左上角放置模式切换，底部附近显示简短提示。在使用鼠标的桌面端隐藏摇杆和大型触摸操作按钮，但保留小型稀有道具控制；在窄屏上支持紧凑的多项触摸操作同时进行。添加带有青绿色深度变化的动态波浪着色器、带弧形棕榈树和岩石的自然沙质小岛、细微的木纹和草地纹理。将每个被困角色包裹在尺寸匹配的半透明泡泡中，泡泡带有虹彩菲涅耳边缘、轻柔漂浮效果、小气泡和地面涟漪。验证姿态下的蒙皮顶点贴合，救援时重置抬升状态，并检查反复切换后的资源释放。避免为水膜使用昂贵的全场景透射渲染过程；将窄屏像素比上限设为 1.5。
