...
async function main() {
  ...
  const { prompts, revision: initialRevision, modelId } = await fetchCollection(cms)
  console.log(`Verified ${prompts.length} published Astra prompts × ${locales.length} complete languages`)
  if (process.argv.includes('--verify-only')) return
  const output = await prepareMedia(prompts, cms, await readManifest())
  for (const [path, contents] of renderAll(prompts)) output.set(path, contents)
  ...
  try {
    ...
    console.log(`Synced ${prompts.length} prompts into 14 catalogs, 2 READMEs and the source-code index`)
